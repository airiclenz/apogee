...
// landlock between fork and execve under CGO_ENABLED=0). Linux's namespace fallback
// launches under bwrap and macOS confines via sandbox-exec — each IS the wrapper process,
// so the apogee binary is never re-invoked — and Windows hands
