...
func probeModelCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			result.Save = recordProbeFingerprint(result, roots, !noSave,
...
// report must say about it: where the record lives and whether an earlier record for the same
// endpoint + label disagreed (a model swapped behind an unchanged label).
//
// It lives HERE rather than in internal/probe because the consequence is composed of things the
// composition root owns — the apogee home's roots — and because keeping the write at wire time is
// the placement discipline the composition root holds for every home-derived decision. A zero
// fingerprint (an incomplete battery) writes nothing at all: an identity may not be minted from
// evidence with a hole in it.
//
// The order matters: the previous-record comparison must read the disk BEFORE the write replaces
// it. A record that was not written (--no-save, a failed write) has no NEW effect to claim; the
// report's effect line says so, naming any earlier record that survives and therefore continues
// to apply.
func recordProbeFingerprint(m probe.Model, roots stateRoots, save bool, printErr func(string)) probe.SaveOutcome {
