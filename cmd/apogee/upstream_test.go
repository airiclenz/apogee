...
func TestRunRootSwitchServerRepointsTheSession(t *testing.T) {
	...
	if err := rec.opts.Sessions.Save(apogee.Session{}, nil, "switched", 1, 0, session.Usage{}, session.Usage{}, nil); err != nil {
...
func TestMoveReFollowsTheParallelAgentsCap(t *testing.T) {
	...
	mover := sessionMover{
		...
		live: newLiveSettings(config.Options{}), keys: config.NewKeyResolver(""), caps: caps,
	}
