...
// session-naming completion behind [tui.Options.GenerateTitle]; naming.go its delegation
// twin — the one completion an unnamed sub-agent's name is generated by, built on the
// CHILD's own Upstream and gated by the same live `auto-title:` switch (ADR 0068);
// modelprofile.go the per-model resolution — which shape a model speaks the wire in
