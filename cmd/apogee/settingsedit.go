...
// read-only, minus the confinement pair (`/confine`). The pane edits every one of them by handing the human
// the file itself, opened in their own editor on that key's line, and re-reading it when they come back.
// Both halves are the binary's, for the reason every other settings seam is (ADR 0011's thin renderer):
// where the config lives, which line a key sits on, which editor this environment names and what the file's
// new text RESOLVES to are all the schema's business, and the renderer that runs the command holds none of
// it.
//
