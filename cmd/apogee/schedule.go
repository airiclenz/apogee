...
// path, and that is precisely the rung a Firing does not have — its Approver denies rather than asks
// (ADR 0033, decision 2), so auto there would be a plan Firing that fails loudly at every write. The
// unconfined case (`confine-to-workspace: false`) is NOT blocked: that is the user's own explicit
// "I am the sandbox", the same ladder that lets the session launch in Auto, and a schedule is never
// held to a stricter bar than a launch (decision 3).
//
...
func scheduleAutoBlocked(backend string, caps apogee.ConfinementCaps, confineToWorkspace bool) string {
	...
	return fmt.Sprintf(
		"the %s backend on this host reports no filesystem confinement, so auto falls back to "+
			"approval — and a firing has nobody to ask", backend)
}
