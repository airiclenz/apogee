...
type delegationWiring struct {
	...
	// targetBinding, targetDialect and targetBound are the dial facts of the target the last
	// non-nil landing resolved — endpoint, key, model and the effort wire shape that server reads —
	// kept for the ONE reader that needs to talk to the Sub-agent server without going through the
	// engine: the out-of-band naming call a routed child's name is generated by (ADR 0068 decision
	// 2, routedBinding).
	//
	// They are recorded on a non-nil landing and never cleared, which is deliberate. A beat that
	// resolves nothing unlatches routing, so the children spawned after it are not routed and their
	// naming calls go to the session's server anyway; the one case a cleared record would change is
	// a child ALREADY running on the target when the box stopped answering the beat — and naming
	// that child on the session's server instead is the worse answer, not the safer one.
	targetBinding upstreamBinding
	targetDialect provider.EffortDialect
	targetBound   bool
}
...
func (d *delegationWiring) land(generation int, name string, target *apogee.DelegationTarget, keyErr error) {
	...
	if target != nil {
		...
		d.targetBinding = upstreamBinding{Endpoint: target.Endpoint, Model: target.Model, APIKey: target.APIKey}
...
// routedBinding answers the Upstream a ROUTED child's out-of-band naming call is built on — the
// Sub-agent server as the last landing resolved it — and false when nothing has ever landed, which
// is a session whose `sub-agents-server:` names nothing, names an entry the list does not carry, or
// has not had its first beat yet.
//
// It is a reader of this holder rather than of the engine's latch for ADR 0031's reason: the host
// resolved those dial facts, and asking the engine to hand them back so the host can build a
// request out of them would put the wiring on the wrong side of the seam. The caller falls back to
// the session's own binding on false (cmd/apogee/naming.go).
func (d *delegationWiring) routedBinding() (upstreamBinding, provider.EffortDialect, bool) {
	d.mu.Lock()
	defer d.mu.Unlock()
