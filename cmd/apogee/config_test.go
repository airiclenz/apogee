...
// The llama-launcher key parses into opts.llamaLauncher exactly as written (ADR 0029): the three
// shapes — absent, `off`, and a path — reach the composition root untouched, because it is the root
// that owns the ladder (auto-detect / disabled / this file) and the launcher it resolves against.
// Absent must stay EMPTY, which is what makes a machine with no launcher behave as it always did.
func TestApplyConfigLlamaLauncher(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name       string
		configYAML string
		...
		{
			name:       "no config file at all ⇒ empty (auto-detect)",
			configYAML: "",
			want:       "",
		},
		{
		...
		{
			name:       "off passes through as written",
			configYAML: "llama-launcher: off\n",
			want:       "off",
		},
		{
			name:       "a path passes through unresolved, and is not checked for existence",
			configYAML: "llama-launcher: /no/such/launcher/config.yaml\n",
			want:       "/no/such/launcher/config.yaml",
		},
		{
			name:       "a tilde path is left for the root to expand",
			configYAML: "llama-launcher: ~/launcher/config.yaml\n",
			want:       "~/launcher/config.yaml",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if opts.llamaLauncher != tt.want {
				t.Errorf("opts.llamaLauncher = %q; want %q", opts.llamaLauncher, tt.want)
			}
		})
	}
}

// The environment cannot point apogee at a launcher config, and neither can a flag: there is no
// APOGEE_LLAMA_LAUNCHER and no --llama-launcher, so a file that says nothing resolves to nothing
...
func TestApplyConfigLlamaLauncherIsFileOnly(t *testing.T) {
	...
	changed := func(name string) bool { return strings.Contains(name, "launcher") }
	opts := options{configDir: home, llamaLauncher: "/from/a/flag.yaml"}
	if err := applyConfig(&opts, changed, getenv, os.ReadFile, noNotify); err != nil {
		t.Fatalf("applyConfig: %v", err)
	}
	if opts.llamaLauncher != "" {
		t.Errorf("opts.llamaLauncher = %q; want empty — the key is config-file only", opts.llamaLauncher)
	}
}

// A llama-launcher value that is no shape the key has is a loud startup error naming the key —
// while a path that simply is not there is NOT, because that is a fact about this machine and it
// degrades at the first command instead (ADR 0029).
func TestApplyConfigLlamaLauncherInvalid(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name       string
		configYAML string
	...
	}{
		{
			...
			configYAML: "llama-launcher: \"   \"\n",
			wantErr:    []string{"llama-launcher:", "whitespace", "off"},
		},
		{
			name:       "a URL, where the key takes a local path",
			configYAML: "llama-launcher: http://192.168.64.1:7331/mcp\n",
			wantErr:    []string{"llama-launcher:", "looks like a URL", "mcp-servers:"},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := applyConfig(&opts, func(string) bool { return false }, func(string) string { return "" }, os.ReadFile, noNotify)
			if err == nil {
				t.Fatalf("applyConfig = nil error; want the value refused (opts.llamaLauncher = %q)", opts.llamaLauncher)
			}
			for _, want := range tt.wantErr {
				if !strings.Contains(err.Error(), want) {
					t.Errorf("error = %q; want it to contain %q", err, want)
