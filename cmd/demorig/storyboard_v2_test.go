...
// v2Fixture is a v2 storyboard fixture under testdata/v2.
func v2Fixture(name string) string {
	return filepath.Join("testdata", "v2", name)
}

func TestLoadV2GoodFixture(t *testing.T) {
	t.Parallel()

	board, err := LoadV2(v2Fixture("good.yaml"))
	if err != nil {
		t.Fatalf("LoadV2: %v", err)
	}

	dir := filepath.Join("testdata", "v2")
	for _, resolved := range []struct{ name, got, want string }{
		...
		{"fonts", board.Fonts, filepath.Join("testdata", "fonts")},
	} {
		if resolved.got != resolved.want {
			t.Errorf("%s: want %q, got %q", resolved.name, resolved.want, resolved.got)
		}
	}
	wantFrame := FrameV2{Cols: 80, Rows: 24, Padding: 16, FontSize: 14, LineHeight: 1.2, Scale: 2, Width: 600, FPS: 24, MaxColors: 128}
...
func TestLoadV2RejectsBadFixtures(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{"bad-expect-without-entry.yaml", "beat 3: expect[0]: entry: required"},
	}
	for _, tc := range cases {
		t.Run(tc.fixture, func(t *testing.T) {
			t.Parallel()

			_, err := LoadV2(v2Fixture(tc.fixture))
...
func TestLoadV2ReportsEveryProblem(t *testing.T) {
	t.Parallel()
