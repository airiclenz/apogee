...
// above calls — the environment scrub and the denial labels — plus RunHookSubprocess, the one
// exported door onto that funnel, so a HOOK that must spawn (an engine-origin Go Reaction) gets
