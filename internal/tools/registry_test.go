...
func TestNewDefaultRegistry_HoldsTheBuiltInTools(t *testing.T) {
	...
	// uses a zero HostTools, so the default set is 22 (18 base/exec/git/diag + 3 network +
	// sub_agent).
	if got := len(registry.All()); got != 22 {
		t.Errorf("default registry holds %d tools, want 22", got)
	}
...
func TestNewDefaultRegistryWithHost_RegistersAskUserOnlyWithAsker(t *testing.T) {
	...
	if got := len(all); got != 23 {
		t.Errorf("registry with Asker holds %d tools, want 23", got)
	}
}
...
func TestNewDefaultRegistryWithHost_RegistersPresentDocumentOnlyWithPresenter(t *testing.T) {
	...
	if got := len(reg.All()); got != 23 {
		t.Errorf("registry with a Presenter holds %d tools, want 23", got)
	}
	...
	if got := len(both); got != 24 {
		t.Errorf("registry with both delegates holds %d tools, want 24", got)
	}
