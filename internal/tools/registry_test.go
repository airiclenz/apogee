...
func TestDefaultTools_DeclareReadOnlyNature(t *testing.T) {
	...
	want := map[string]bool{
		...
		// harmless read. Its read-only DECLARATION is an honest statement about the tool; what
		// decides the class is the readOnlySubprocess marker it carries (§4 amended 2026-09-06),
		// which the ladder gives the read-only row in every mode — Plan offers and runs it, Auto
		// runs it unconfined.
		"git_branch":     false,
		"git_commit":     false,
		"git_diff_range": true,
		...
		"diagnostics": true,
		// run_tests (2026-08-10): a suite runs the project's OWN code, which writes — so unlike
		// diagnostics it declares no read-only nature and takes terminal's disposition.
		"run_tests": false,
