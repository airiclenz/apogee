...
// /rename and /schedule — complete instead. /model, /server and /skills take arguments too, but
// their bare form is the whole verb — a picker for the first two, the catalog report for /skills —
// so they carry runsBareAtAccept and RUN like the rest.
...
//     finished would be wrong. /model, /server and /skills read arguments as well, but their bare
//     form is the whole verb (a picker that mutates nothing until its own accept; for /skills, the
//     catalog report), so runsBareAtAccept puts them back among the rows that run;
