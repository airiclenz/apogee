...
func (c *tokenConfiner) Close() error {
	err := c.journal.Retire(revertSparingLiveSiblings)
	if c.token != 0 {
		_ = c.token.Close()
		c.token = 0
	}
...
// to undo the mutation, and what it may say about a root is the journal's own decision —
...
// so its just-journalled no-prior entry is removed again (winlabel.Journal's Unwind) rather
// than left to alarm forever as residue for a label that never landed.
//
...
func (c *tokenConfiner) labelBox(box domain.ConfinementBox) error {
	...
	for _, root := range roots {
		...
		journalled, err := c.journal.Record(winlabel.Entry{Path: root, Root: true, PriorSDDL: prior})
		if err != nil {
		...
		if err != nil {
			...
			// root label may really be on the disk. What the unwind may remove is likewise the
			// journal's decision — an entry with a foreign prior is kept.
			if journalled && !rootLabelled {
				c.journal.Unwind(root)
			}
			return err
		}
		c.journal.MarkLabelled(root)
	}
	return nil
}
...
func (c *tokenConfiner) labelTree(root string) (rootLabelled bool, err error) {
	...
	return true, filepath.WalkDir(root, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		if shouldJournal {
			...
			if _, err := c.journal.Record(winlabel.Entry{Path: path, PriorSDDL: prior}); err != nil {
				return fmt.Errorf("%w: %v", domain.ErrConfinementUnavailable, err)
			}
		}
...
// a live owning process still names (winlabel.RevertibleRoots), restoring only the priors no sibling
// journal still claims the tree of — the rest are handed back as the journal's remains
// (winlabel.RestorablePriors, winlabel.Retire). Teardown and recovery both revert through this
...
func revertSparingLiveSiblings(home, own string) func(winlabel.Record) ([]winlabel.Entry, error) {
	return func(j winlabel.Record) ([]winlabel.Entry, error) {
		...
		restore, handoff := winlabel.RestorablePriors(j, siblings)
		if err := revertLabelJournal(winlabel.RevertibleRoots(j, siblings, processAlive), restore); err != nil {
			return nil, err
		}
		return handoff, nil
	}
...
// minus what a live sibling session still claims (winlabel.RevertibleRoots) — a spared root is not a
...
// subset (winlabel.RestorablePriors): a prior under a sibling-claimed root is handed off rather than
...
// through winlabel.ClearTreeOutcome, because a nil return here is what winlabel.Retire takes as
...
func clearLabelTree(root string) error {
	...
	return winlabel.ClearTreeOutcome(root, failures, first)
}
...
// gone.
//
// A journal whose revert fails survives this pass (winlabel.Retire): recovery is
...
// because a sibling journal still claimed the root (winlabel.RestorablePriors) becomes completable
...
func recoverLabelJournals(home string) {
	...
	for {
		...
		for _, path := range winlabel.ListJournals(home) {
			...
			remaining, err := winlabel.Retire(path, j, revertSparingLiveSiblings(home, path))
