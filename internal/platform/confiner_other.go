...
// (the three that have one — Linux landlock with its namespace fallback through bwrap, macOS
// seatbelt, the Windows restricted-token backend — each select their own): denyConfiner, which reports {false, false} so the
