...
type Entry struct {
	...
	// Carried counts the reverts that have found this prior's path without apogee's own Low
	// mark — a FOREIGN label put there since the run that journalled the prior, or no label at
	// all. That verdict is neither a restore (nothing of apogee's is
