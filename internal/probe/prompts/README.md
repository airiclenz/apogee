...
`BatteryVersion` (`internal/probe/battery.go`), the one constant `SaveProbeRecord` stamps into
every record and `LoadProbeRecord` compares against (`internal/probe/proberecord.go`), so the
bump retires every incomparable record. The files carry no comments and no header of
