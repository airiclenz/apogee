...
type ApprovalRequest struct {
	...
	SubAgentTask string
	// SubAgentName is the OPTIONAL short name that child is known by — the few words a human
	// recognises it by where SubAgentTask is a whole sentence. It is the name the spawning
	// sub_agent call supplied, or the one the out-of-band namer generated for a call that
	// supplied none (ADR 0068), read at the moment this request was built. It is DISPLAY
	// identity only, never privilege (ADR 0005): a prompt reads better for it, nothing is
	// decided by it.
	//
	// It is empty whenever the delegation has no name (and always at depth 0), which is the
