// Package domaintest is the Reaction seam's shared test adapter (the
// internal/platform/confinetest precedent): conversation fixtures and a settable
// LoopView fake, so a Reaction or engine test builds history and loop state through
...
// the Reaction set is curated; the domain seam, not this adapter, is the surface).
//
// Three pieces:
//
...
//     Call and ReadCall (the read_file shape the read-counting Floor guards inspect).
