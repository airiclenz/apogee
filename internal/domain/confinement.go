...
type ConfinementCaps struct {
	...
	// below is set wherever FSWrite is false, including in exactly those cells, and is what a
	// caller reads to tell the reasons apart. It is the other half of capability honesty
	// (contract §5): a backend that cannot fence says so AND says why, so the user diagnosing
	// a gating Auto is told which host fact to change. It is disclosure only — AutoEligible reads FSWrite alone and never
	// this.
	Unavailable string
	...
	// that cannot fence sets one — including the three whose sentence is empty — and it is
	// empty exactly when FSWrite is true. Disclosure only, like the sentence: AutoEligible
	// never reads it.
...
//     Which contexts carry one is the MINTING SITE's row rather than this type's, and the engine
//     has ONE such site: a USER-ORIGIN SYNC reaction's permit is minted in EVERY mode
//     (syncPermitCtx), because its command is the user's own configuration rather than anything
//     the model chose, and it is withheld only where the box it would have to run inside cannot be
//     built at all. No cascade installs one — the post-response cascade's Auto-only row was
//     retired 2026-09-15 (no shipped Reaction spawns at that Moment), so every handler a cascade
//     runs reads absent.
//   - PRESENT with a nil Confinement: run UNFENCED — the confine-to-workspace:false "I am the
//     sandbox" opt-in, mirroring resolveLadderAuto's unfenced row.
...
// spawn a subprocess on the terms p describes. Only the engine installs it, and only at the one
// site whose own row permits execution: a user-origin sync reaction in every mode (syncPermitCtx,
