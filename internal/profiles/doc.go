...
//
// The confidence gate of the fingerprint ladder is deliberately NOT reused here:
// confidence is a fingerprint property that needs the explicit model probe battery
...
// stripping, one config line to override) with no bench-honesty claim riding on it.
//
// The package imports internal/domain only and decides nothing: it maps a model name to a
// Decision, and the composition root (cmd/apogee) applies it and emits the shipped-match
// notice, so the engine and the bench never see this package.
//
