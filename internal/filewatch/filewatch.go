// Package filewatch reports that one file on disk changed, and nothing else about it: the
// stat-poll mechanism of ADR 0041 decision 3, in a home both of its callers can hold.
//
...
//
// This package is that mechanism and nothing else. It reads no YAML, holds no projection and applies
...
//
// It sits beside internal/config rather than inside it because the poll has a second caller that
// watches a different file: the daemon live-reloads `~/.apogee/daemon/schedules.yaml` (ADR 0034) on
// exactly this cadence, settle window and pair of witnesses. One implementation, two callers — the
// sibling copy that would otherwise be written for the second file is what this package exists to
// prevent. What it still refuses to hold is a policy about WHICH files are watched or what a report
// means: skills, MCP manifests and context files each have their own reload semantics (ADR 0041
// decision 5 — one file, one contract), and that contract belongs to the caller that owns the file.
package filewatch
...
// defaultInterval is the poll cadence. One os.Stat a second against a file that changes a
...
// defaultSettle is how long the file must hold still before a change is reported. An editor
...
// fileState is what a poll can learn about the file cheaply: when it was last written, and how
...
type Watcher struct {
	// path is the single file this watcher is over, and one watcher watches exactly one file: a
	// caller wanting two files under watch holds two Watchers, because "something changed" over a
	// set of files is not an answer either caller could act on.
	path string
	// Interval is the poll cadence and Settle the quiet period a change must survive before it is
	// reported. New sets both to the production constants above; they are exported and
...
// reports once.
func (w *Watcher) poll(last fileState) {
	defer close(w.done)
...
func (w *Watcher) sample() (fileState, bool) {
