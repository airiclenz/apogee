...
// that an entry's enable set is whole against the current Mechanism roster
// (whole-set-or-nothing: a subset is an unvalidated stack and must not apply).
//
...
//
// The package is deliberately mechanism-catalogue-agnostic: set members are plain strings
// and the caller passes the known roster in. The decision to apply lives
