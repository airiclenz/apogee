...
// engine made at construction (the wire model id, the system-prompt template, the context
// window) then describes a model that is no longer there. Rebind is
...
// model gets (ADR 0023) is a config question resolved in cmd/apogee, so the engine neither reads
// config.yaml nor re-derives an identity — it applies what it is handed, atomically.
//
...
//
// Validate-then-commit: every binding the spec carries is validated BEFORE anything is mutated,
// so a spec the new model cannot satisfy leaves every existing binding, and the whole
...
// system-prompt template and model profile are all facts only that server can report, so the
...
// confinement flag, and the resolved tools — none of them describe a server. The model profile
// also stands, still describing the model that just went away, until the follow-up Rebind
// re-resolves it for the new one; it is unreachable meanwhile, since no request can open while
// nothing is bound.
