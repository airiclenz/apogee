...
// TestRetryView_RepeatedRepairFailGetsCorrectionNotToolLoop: a model that repeats its
// repair-rejected call gets the repair correction again on the second retry — NOT the tool-loop
// breaker's escalation. The repeat only matches the SUPERSEDED first attempt (in the appendage),
// never a committed turn, so the breaker must stay inert with the fix. Both guards run at the same
// seam here, the breaker FIRST, so this also proves the ordering does not mis-fire on a retry.
func TestRetryView_RepeatedRepairFailGetsCorrectionNotToolLoop(t *testing.T) {
	sink := &recordingSink{}
	...
	if n := guardFireCountFor(sink.events, guardToolLoopBreaker); n != 0 {
		t.Errorf("the loop breaker fired %d times; the superseded call must not read as the previous turn", n)
	}
	// Both attempts were rejected by the repair guard, so it retried twice — the model kept getting
	// the correction, never the loop directive.
	if n := guardFireCountFor(sink.events, guardToolCallRepair); n != 2 {
		t.Errorf("the repair guard fired %d times, want 2 (both rejected attempts got the correction)", n)
	}
	...
	if wireUserIndexContaining(third, "STOP. You are in a loop") >= 0 {
		t.Errorf("a loop-breaking directive reached the model — the superseded repeat wrongly read as a committed loop: %+v", third)
	}
}
