...
// until the fill either crosses the next rung or drops back under a fired one, when a fold has
// re-armed it (a.fillRung). A zero fill is an unknown window or an uncalibrated ratio, and the
...
func (a *Agent) contextFillNotice(_ context.Context, view domain.LoopView, _ domain.ToolCall, result *domain.ToolResultEdit) (domain.Outcome, error) {
	...
	case reached < a.fillRung:
		a.fillRung = reached // the fill dropped back under a fired rung: re-arm it
		return domain.Outcome{}, nil
	case reached == a.fillRung:
		return domain.Outcome{}, nil
	}
