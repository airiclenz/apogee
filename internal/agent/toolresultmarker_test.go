...
func TestAppendToolResultCommitsTheOutcomeMarker(t *testing.T) {
	...
	// backwards.
	a.appendToolResult(0, domain.ToolCall{}, "", domain.ToolResult{CallID: "ok", Content: "[File: e.go]\nreturn fmt.Errorf(\"error: does not exist\")"}, "", nil)
	a.appendToolResult(0, domain.ToolCall{}, "", domain.ToolResult{CallID: "bad", Content: "gone.go", IsError: true}, "", nil)
...
func TestToolOutcomeMarkerSurvivesSnapshotResume(t *testing.T) {
	...
	a.appendToolResult(0, domain.ToolCall{}, "", domain.ToolResult{CallID: "c1", Content: "no such file: gone.go", IsError: true}, "", nil)
