...
// The intent classifier ported inline from apogee-sim internal/intent/intent.go @pin. It is a
// shared helper rather than a guard of its own: it fires at no seam and decides nothing by itself.
// It is a cheap lexical heuristic — does the user's last message ask for an ACTION (edit/create/run
// …) as opposed to ANALYSIS (summarize/explain …) or a question. Its consumer is the tool-use
// enforcer, which reads it to decide whether narrating instead of acting is a failure worth
// correcting (an analysis request answered in prose is fine; an action request is not).
