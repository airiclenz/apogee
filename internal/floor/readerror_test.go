...
// normalizePath makes the two spellings of one file compare equal, which is what lets a guard
// recognise a path the model has already read.
...
// contentMatchesAny scans the whole text and firstLineMatchesAny only the first non-blank line —
// the difference that keeps a source file's own error strings from reading as a failed read.
func TestSignalMatchingIsAnchoredOnTheFirstLine(t *testing.T) {
	t.Parallel()

	body := "package main\n\n// nothing here does not exist any more\n"

	if !contentMatchesAny(body, readErrorSignals) {
		t.Error("contentMatchesAny = false, want true — the signal is in the body")
	}
	if firstLineMatchesAny(body, readErrorSignals) {
		t.Error("firstLineMatchesAny = true, want false — the first line carries no signal")
	}
	if !firstLineMatchesAny("\n\nERROR: no such file\npackage main", readErrorSignals) {
		t.Error("firstLineMatchesAny = false, want true — blank lines lead, the signal follows")
	}
	if firstLineMatchesAny("", readErrorSignals) {
		t.Error("firstLineMatchesAny(\"\") = true, want false")
	}
}
