...
// (a synthesized call names nothing). Both spellings go through sanitize.FirstLine, so a name is
// folded to one line exactly as a task is.
//
...
func DelegateLabel(name, task string) string {
	if line := sanitize.FirstLine(name); line != "" {
		return line
	}
	return sanitize.FirstLine(task)
}
