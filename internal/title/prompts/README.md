# The naming call's prompt assets — editing one re-words every session name

Every `.txt` file in this directory is prompt text the naming call sends to a live model, and the
...
run oldest first. Re-word one and every session named from then on is named by a different
...
keep saying, and `TestUserInstructionAndWindowHeaderPinTheirExactWording` holds the two one-line
assets byte-for-byte, both in `internal/title/title_test.go`. Their literals are deliberately
duplicated rather than derived from the assets: a pin that fails is the question "did you mean to
re-word this?", not a bug.
