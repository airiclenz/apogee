...
func TestShellWriteViewJudgesWhatTheCommandWrites(t *testing.T) {
	...
	cases := []struct {
		...
		{"git config writing a command-valued key into the repo's own config", terminalCall("git config --local filter.x.clean cmd"), shellTool, TierHardRefuse, "write-git-control-plane"},
		{"git config setting core.hooksPath", terminalCall("git config core.hooksPath /tmp/h"), shellTool, TierHardRefuse, "write-git-control-plane"},
		{"git config of a plain key stays clear", terminalCall("git config user.name x"), shellTool, TierNone, ""},
