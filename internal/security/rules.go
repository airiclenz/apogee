...
//
// Removal is by rule ID; an unknown remove-ID is ignored. An added rule with an ID that
// already exists replaces the earlier rule (so a project may tighten a default's reason
// or pattern, but the result is still at least as strict — it cannot remove). The merged
// slice is returned; pass it to NewDangerousActionGuard.
func MergeDangerousRules(base, globalAdd []Rule, globalRemove []string, projectAdd []Rule) []Rule {
	...
	add := func(rules []Rule, honorRemove bool) {
		for _, r := range rules {
			if r.ID == "" {
				continue
			}
