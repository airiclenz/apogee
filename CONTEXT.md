...
**Undo journal**:
The engine's per-[Exchange](#turns-and-stepping) record of what the agent's writes **replaced**,
...
**skipped and reported**: the human's own later edit outranks the undo. Groups materialize
**lazily**, an [Interjection](#turns-and-stepping) opens none, and a sub-agent writes into its
parent's. It is **persisted host state, never [Session](#identity-and-shape) state** (ADR 0022 §8):
kept per session id, reopened under the id `/new`, `/clear` or a resume puts in force. See
[ADR 0074](docs/adr/0074-undo-is-a-per-exchange-snapshot-pair-in-a-session-owned-git-object-database.md).
...
"rollback" (that word belongs to the abort-rollback boundary in [Exchange](#turns-and-stepping)).
...
to `/confine off --save`. Four keys are written off a gesture outside the pane, each admitted by
name: `server:` (ADR 0036 decision 2), `launch-profile:` under `remember-model:` (ADR 0048),
