...
- Run `make check` before committing; a multi-item skill run satisfies this once, at its closeout — per-item commits run the item's targeted acceptance instead. `APOGEE_TEST_SHARDS`, `APOGEE_TEST_RACE=0` and `APOGEE_TEST_SLOW=1` tune `make test` for a slow box (`make check` refuses `RACE=0`); see `docs/manual/building.md`.
