...
"apogee" (spelled lower-case) is a terminal AI coding agent (Go, Bubble Tea v2 TUI) built for smaller, locally hosted LLMs — and working even better with bigger ones — running a full agentic tool-use loop over any OpenAI-compatible server or the Anthropic Messages API. Its hard invariant: nothing apogee puts in front of a model may make that model perform worse than the bare loop. The seven **Floor guards** and the structural reducers that ship on for every model are held to it; any model-facing behaviour above them (the **Reaction** core — [ADR 0076](docs/adr/0076-one-reaction-core-with-an-origin-by-class-policy-matrix.md); **Bypass mode** switches its model-shaping classes off) ships off until bench evidence turns it on. This file is the agent-facing counterpart to `README.md`: it maps where knowledge lives and states the conventions you cannot derive from the code.

## Where knowledge lives

- `CONTEXT.md` — the domain language and concept map (Floor guards, Reactions, Moments, Bypass, Drivers, Steps, Turns, …). Terms used in code and docs are defined here; read the relevant section before design work. Large — read sections, don't load it wholesale.
...
- `docs/design/` — design contracts (confinement execution contract, MCP client, the `reaction-core-greenfield.md` collapse) plus the `tool-surface-findings.md` and `hook-talkback-findings.md` records; the mechanism catalogue under `docs/design/archived/` carries the retirement wave's per-row verdicts (ADR 0071) — historical, since the Reaction core superseded that surface (ADR 0076).
...
- **No AI attribution trailers** in commit messages (no Co-Authored-By / "generated with" / session-link lines); a commit-msg hook is the backstop, never the rule.
- **Git hooks live in `.beads/hooks/`, not `.git/hooks/`.** `bd hooks install` points `core.hooksPath` there, making it the live path — a fresh clone has not run it, so check `bd hooks list` before relying on any hook. Running it is a trust decision, not a formality: from then on every `git commit`, `checkout` and `push` runs this checkout's shell with your full user privileges, and apogee fences none of it (`SECURITY.md`, "What counts") — so hydrate only a checkout you trust, and in a clone of a fork or of a repository you do not control read `.beads/hooks/` before you do. A checkout may keep a byte-identical copy of `.beads/hooks/commit-msg` at `.git/hooks/commit-msg` as a dormant fallback for when `core.hooksPath` is unset. bd manages five of the six hooks (`pre-commit`, `post-merge`, `pre-push`, `post-checkout`, `prepare-commit-msg`) inside `BEGIN/END BEADS INTEGRATION` markers and preserves anything written outside them; `commit-msg` carries no markers and is absent from `bd hooks list`, so the attribution stripper survives a `bd hooks install`. `export.git-add` is `false` (bd's default), so the pre-commit hook writes `.beads/issues.jsonl` and stages nothing — and it exports at all only when a `.beads/` path is already staged (`pre-commit: skipping JSONL export — no staged .beads paths` otherwise). **The staged-file sweep does not reproduce** (2026-09-04, throwaway clone hydrated with `bd init --prefix apogee` + `bd import` until `.beads/embeddeddolt` existed and the hook was seen rewriting `.beads/issues.jsonl`; bd shim 1.2.2): commits carrying a staged deletion plus a staged edit — with and without a staged `.beads/` path, and with unstaged and untracked files present — landed exactly the index and nothing more, so ordinary commits need no `--no-verify`. What did reproduce is **`bd init`'s own auto-commit**: run in a clone with a deletion and an edit already staged, its "bd init: initialize beads issue tracking" commit swallowed both — that, not the pre-commit hook, is what took the retirement wave's 22 staged deletions. Never run `bd init` with a dirty index — and since 2026-09-05 a guard refuses it for you (apogee-f82): the tail of `.beads/hooks/prepare-commit-msg`, written **outside** the `BEGIN/END BEADS INTEGRATION` markers so `bd hooks install` preserves it (verified), aborts any commit whose subject is `bd init: initialize beads issue tracking` while the index holds paths bd does not own (anything outside `.beads/`, `.agents/`, `.claude/`, `.codex/`, `.gitignore`, `AGENTS.md`, `CLAUDE.md` and the other agent-instruction files bd init writes). `prepare-commit-msg` carries it because **bd commits with `--no-verify`**: neither `pre-commit` nor `commit-msg` runs for that commit, and `prepare-commit-msg` is the only hook git still fires that both reads the message (`$1`) and aborts on a non-zero exit. bd swallows the hook's stderr, so the refusal is written to `/dev/tty` as well; what bd itself prints is `Warning: failed to commit beads files: exit status 1`. An ordinary commit, a legitimate bd-only commit and a `bd init` against a clean tree all pass untouched. One consequence to know: because the hook re-exports *after* git has snapshotted the index, committing a staged `.beads/issues.jsonl` commits the pre-hook content and leaves the fresh export unstaged — re-stage and amend when the export itself is the point of the commit.
- Run `make check` before committing; a multi-item skill run satisfies this once, at its closeout — per-item commits run the item's targeted acceptance instead. `APOGEE_TEST_SHARDS`, `APOGEE_TEST_RACE=0` and `APOGEE_TEST_SLOW=1` tune `make test` for a slow box (`make check` refuses `RACE=0`); see `docs/manual/building.md`. **`make test` is the supported way to run the suite**; a raw multi-package `go test` — `./...`, or two heavy packages named on one line — is not, because `go test` runs packages concurrently and gives each of them the whole box for its own `t.Parallel` fan-out, which nothing then bounds: the driven tests in `cmd/apogee` and `internal/tui` land on a machine loaded by their neighbours and report that load as a failure. To debug, narrow to one package and one test (`go test -race -count=1 -run TestX ./cmd/apogee/`), which gets the box to itself.
...
- Distribution: a Homebrew tap (`airiclenz/tap`, binary formula) plus six prebuilt archives per release (`make dist`); building from source stays fully supported. `go install …@latest` resolves to the current 0.x release since 2026-09-16: `go.mod` carries `retract [v1.0.0, v1.8.0]`, published in the self-retracting `v1.8.0` tag whose only job is to retract the deleted v1.x series that proxy.golang.org retains immutably (a retraction is read from the go.mod of the *highest* version, so it could not ride a 0.x tag). Keep the `retract` block in `go.mod`; never publish a version above v1.8.0 without it.
- Config home is a single `~/.apogee` dotdir on every OS (like `~/.aws`). Settled decision — do not propose XDG or `os.UserConfigDir`.
...
- **Public docs speak the shipped vocabulary.** `reactions:` (never `hooks:`, except in the migration notes), `wire: anthropic`, the four modes `plan | ask-before | allow-edits | auto`; a count the README states (34 tools, 30 on the default menu; seven Floor guards; sixteen Moments) is pinned by a test or a literal in `internal/tools/registry.go`, `internal/config/reactions.go` and `internal/domain/reaction.go` — check there before changing it.
...
- **Every new bead gets a spoken id: `bd create --id apogee-<slug>`.** Without `--id`, bd mints a hash suffix (`apogee-242`, `apogee-qi3`) that says nothing in a plan, a commit message or a `bd ready` listing; with it, any slug is accepted verbatim (`apogee-net-deny-egress`). The slug is two to four kebab-case words naming the defect or the work, never the fix. There is no ID-generator setting — `issue_prefix` is bd's only ID knob — so the flag is the whole mechanism. It exists on `bd create` alone: `bd q` has no `--id`, and neither do the batch forms (`--file`, `--graph`), so a bead that needs a spoken id is created singly with `bd create` (verified against bd 1.3.0, 2026-09-22). **An id is permanent:** bd has no rename and `bd update` has no `--id`, so the only way to change one is delete-and-recreate, which drops the bead's comments, history and dependency edges — pick the slug deliberately, and `bd show apogee-<slug>` first if it might collide. The 220 beads created before this rule keep their hash ids and are **not** migrated, so both shapes are live: an `apogee-qi3` in a plan, a changelog or a commit is correct as written, not a stale reference to fix.
