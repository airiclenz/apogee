...
- **Scheduled prompts** — `/schedule` runs a prompt on a cycle for as long as apogee
  ...
  off the clock.
- **Schedules that outlive the session** — `apogee daemon` runs the prompts declared in
  `~/.apogee/daemon/schedules.yaml` for as long as your OS keeps it up, reloading that file
  as you edit it and saving every firing into the same `/sessions` history;
  `apogee daemon install` writes this host's supervisor unit. See
  [Standing schedules](#standing-schedules--apogee-daemon).
...
approve (see [Auto mode's blast radius](#auto-modes-blast-radius), and `apogee probe`
