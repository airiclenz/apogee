...
webhook under a reaction's `run:` is never inspected. Three things are deliberately not reported: the
keys the migration above folds or refuses, which are handled before this check runs; a retired
...
engine's own built-in advise reaction — it is switched with its own top-level key, never with an
...
conversation itself. When the history outgrows **70%** of its budgeted share, apogee
...
history is back under **50%**; the model can always re-run a call whose output it still
needs. The six most recent tool-calling turns are never touched, so the work in progress
...
Every sub-agent is also **told once when its step budget is running out**. The tool result that
closes the turn spending three quarters of the ceiling — the 60th at the default of 80, or three
quarters of the lower number a `max_steps` ask set — carries one extra line, fenced as the
engine's own note (`[engine — step budget]`), never as reaction advice:

```
steps: 60 of 80 used — 20 left before the wrap-up Turn; write your output now
```

It is part of the cap's contract rather than a reaction, so there is no key for it: it is on for
every bounded delegation, stays on under `--bypass`, never fires for the session you are talking
to (which has no cap) or for an unbounded delegation (`delegate-max-steps: 0`), and a compaction
that folds the line away lets it fire again on the next tool result past the threshold. An earlier
build gated it behind a `step-budget-notice:` key; a file that still carries that key is ignored
without a notice.
...
A sub-agent that **faults** — its server dies mid-task, say — is kept the same way: the engine
...
`read_file`, `grep` (as its `path` or one of its `paths`), `list_dir`, `find_files`, `copy_file`'s
source and `present_document` all accept the announced path, a symlinked `~/.apogee` included — so a report the model drafts
