...
| [Reactions](reactions.md) | Commands and webhooks apogee runs on what a session did (`run:`), commands or webhooks that advise the model on a tool result (`advise:`) and commands or webhooks that gate a tool call (`gate:`): the sixteen Moments, the payloads, the exec posture, the webhook contract, migrating from `hooks:` |
