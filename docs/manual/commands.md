...
`tool-result-cap`. The [`context-fill-notice`](configuration.md#context-fill-notice) row beside them
is the same kind of row — `on`/`off`, applied to the running session the moment you commit it — but
it is not a Floor guard and starts `off`.
