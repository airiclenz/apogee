...
| `/sub-agents-server` | Pick the `servers:` entry this session's delegations run on — picker, or `/sub-agents-server <name>`; the choice is recorded and the next delegation goes there. The picker's last row is `auto`, and taking it — or `/sub-agents-server auto` — clears the recorded key and runs delegations on this session's own server — see [below](configuration.md#the-servers-you-run-models-on) | ✅ |
...
**The blocks no row can hold open your editor.** `servers:`, `mcp-servers:`, `validated-sets: alias:`,
`system-prompt-models:`, `system-prompt-layers:` and the model profile render as a summary with an
