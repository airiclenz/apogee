...
## 21. Code comment and emitted-string sweep — ✅ DONE (2026-09-09)

NOTES (2026-09-09): the sweep's rule was read as the item words it — a comment or string that calls a USER Reaction a hook, names the `hooks:` key, `APOGEE_HOOK_*` or `approval-waiting`. Four categories of surviving `hook` are therefore left standing, deliberately: (a) the IN-PROCESS seam vocabulary of the hook-mutation API (`internal/domain/hooks.go`, `tooledit.go`, `hookview.go`, `apogee.go`'s facade doc, `internal/agent/loop.go`, `internal/agent/wire.go`, `internal/floor/resultcap.go`) — those comments document identifiers and a design doc the item puts out of scope, and stage 3 owns that surface; (b) "hook" in senses that are not a Reaction at all — the config registry's validate hooks, the tool presenter's stat/body/outcome hooks, `/settings`' parseArgs hooks, the confinement teardown hook, the Cobra flag hook, the auto-title hook, a test's answer hook; (c) migration-compatibility text in `internal/config/configmigrate.go` and its test fixtures, plus the migration journey in `cmd/apogee/e2e_reactions_test.go`, which must name the retired `hooks:` key, the retired `approval-waiting` spelling and `APOGEE_HOOK_*` for the note to mean anything; (d) identifiers, file names, URLs and testdata script names (`w.hooks`, `hookMarkers`, `hooks.go`, `hooks.yaml`, `hooks.example.com`).
NOTES (2026-09-09): the `approval-waiting` literal in `internal/config/configmigrate.go`'s comments and `configmigrate_test.go`'s fixtures is EXEMPT (the DECISION's re-decide): the migrator's whole job is to recognise the retired spelling and say so in the note it writes, so rewording it would make the code describe something it does not do. Same verdict for `APOGEE_HOOK_*`, which survives only inside the sentence "Scripts must read APOGEE_REACTION_* (was APOGEE_HOOK_*)".
NOTES (2026-09-09): the two stale comments the DECISION names are fixed — `internal/tui/doc.go` and `internal/tui/tui.go` no longer list "the validated set (ADR 0016), the mechanisms registry" among what a rebind re-resolves; both now name the per-model system prompt, the window pin and the compaction budget, matching what `rootWiring.rebind` actually does.
NOTES (2026-09-09): consequential edit — cmd/apogee/headless.go: made necessary by the same staleness the DECISION's two comments carry; its rebind-narration comment still promised "a validated set applying, being offered or being suppressed", a surface item 15 deleted, and now names the notices `rebindSpecFor` really returns (a built-in Model profile and its roster delta).
NOTES (2026-09-09): consequential edit — internal/reactions/hooks.go: made necessary by the sweep's own rewrite of the neighbouring sentence; the comment pointed at "internal/config's hooks.go", a file that is `internal/config/reactions.go`.
NOTES (2026-09-09): comment paragraphs whose wording this sweep changed were re-wrapped to the file's ~100-column comment width, since "Reaction" is four characters longer than "Hook". Only paragraphs the sweep actually touched were re-wrapped; no untouched paragraph was reflowed.
NOTES (2026-09-09): `cmd/apogee/wire_verbs.go:26` carries a ~150-column comment line left unwrapped by an earlier item of this run. Cosmetic, in a file this item does not otherwise touch, so it was left alone rather than restyled.
