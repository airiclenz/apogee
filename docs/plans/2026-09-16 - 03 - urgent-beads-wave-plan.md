...
## 6. Denial kill matches a line-anchored signature (apogee-1mj) — ✅ DONE (2026-09-16)

NOTES (2026-09-16): no prose-only subprocess test existed to replace — `TestRunSubprocessDenialWatchIgnoresStdout` was added beside `TestRunSubprocessDenialWatchKillsConfinedRun`, which keeps killing on a stderr denial.
NOTES (2026-09-16): internal/console/process_test.go needed no edit — `TestProcessConfinedDenialStopsTheConsole` stays green as written (its PTY `\r\n` line matches under the `TrimRight` rule); internal/console/process.go gained a comment naming the single-stream exception instead.
NOTES (2026-09-16): consequential edit — internal/platform/doc.go: made necessary by the stderr-only watch and anchored signature (its denialkill.go map line described the old rule).
NOTES (2026-09-16): `TestDenialKillWriterMatchesAcrossWriteBoundary` became a two-case table (split inside the signature, split inside a long allowed tail); its "no kill on a half signature" assertion moved to `TestDenialKillWriterHalfSignatureDoesNotKill`, and `TestDenialKillWriterIgnoresPhraseMidLine` pins the anchoring live.
NOTES (2026-09-16): `bd close apogee-1mj --reason="<commit subject>"` is the verifier's at the commit (standing requirement); not run here.
...
## 9. Anthropic codec — request encoder and whole-response decoder (apogee-6fp) — ✅ DONE (2026-09-16)

NOTES (2026-09-16): `headers()` carries `x-api-key` and `anthropic-version` only — the JSON content type the item lists is already set by `Client` on every wire (client.go), so the codec does not repeat it.
NOTES (2026-09-16): `Sampling.RepeatPenalty` is dropped on this wire (the Messages API has no such field); `thinking` blocks in a reply decode into `RawResponse.Thinking`; an assistant message with neither text nor calls is dropped (the API refuses an empty content array); tool-call prose in the no-tools fold is `name(arguments)`, one per line after the text.
NOTES (2026-09-16): `selectCodec` is untouched — `anthropicCodec` has no `parseSSE` yet, so it cannot satisfy `wireCodec` until item 10 adds the parser; the codec is reached in tests directly.
NOTES (2026-09-16): consequential edit — internal/provider/doc.go: made necessary by the second codec file (the package doc named only wire_openai.go).
NOTES (2026-09-16): follow-up bead filed: apogee-4kl (signed-thinking carrier), cited in the codec's file comment; `.beads/issues.jsonl` carries the export but stays off FILES like the earlier items of this run (it also holds sibling beads' state).
NOTES (2026-09-16): observation, no action — per the current Anthropic API reference, `thinking: {"type":"disabled"}` combined with `output_config.effort` xhigh/max is refused (400) on Opus 5 and `disabled` is refused outright on Fable 5; the ratified "always disabled" call ships as written, the enriched effort hint names the field, and apogee-4kl is the path that lifts it.
...
## 18. Transcript prefix and fork eligibility (apogee-zci) — ✅ DONE (2026-09-16)
NOTES (2026-09-16): the mark is gated on the worker's mailbox (`m.worker.box != nil`) inside both folds — a cancelled `/compact` returns through `foldCancelled` too (`startCompact` → `cancelledMsg{}`) and must not mark the completed prompt before it; the plan's "only on foldCancelled/foldLoopError" holds, with a fourth test case pinning the /compact exclusion.
NOTES (2026-09-16): limitation, not a deviation — the two other `AbortExchange` callers (a fresh message or `/clear` on a session resumed mid-Exchange, model.go submit/startNewSession) drop the interrupted prompt from the engine without marking it, per the binding "only the two folds" rule; such a resumed record forks one Exchange later than picked, exactly as a pre-mark record does (CutSession's range check still bounds it). Improvement idea only, no bead.
NOTES (2026-09-16): the prompt whose own Step ran an automatic fold is not offered (it precedes the compacted entry in the scrollback though its Exchange is intact) — the literal "after the last depth-0 compacted entry" rule, documented as the conservative side on `forkPoints`.
...
## 19. Host seam: `SessionHost.Fork` and the carried parent id (apogee-zci) — ✅ DONE (2026-09-16)
NOTES (2026-09-16): deviation — the user-message rule is spelled once as `session.UserMessageCount` (internal/session/transcript.go, not in the item's Files): the host, the fake host and the queue's assembled record all count the prefix by it (every `user` entry at any depth, the `userMessageCount` rule on the wire form), so the child's cell holds across its first Save without three loops restating the rule.
NOTES (2026-09-16): the queue's completion Record takes its Meta from the child Meta the host returned (UserMsgs included) rather than recounting the prefix in the queue — one source for the child's metadata; the record's transcript is encoded in the queue from the same entries the host encoded (`forkRecord`), and `recordWriteDoneMsg` gains a `fork forkResult` field (internal/tui/messages.go, not in Files — the Msg type lives there).
NOTES (2026-09-16): `parent.ID` serves as the fallback parent pointer only when the host holds no identity at all (no active session and no pre-minted id); otherwise the host's own identity wins, per the regression guard.
...
## 20. `/fork` — picker, cut, switch (apogee-zci) — ✅ DONE (2026-09-16)
NOTES (2026-09-16): deviation — internal/tui/sessionsave.go joins Files: the fork write's completion is folded in `foldRecordWrite` (the one fold every non-save write returns through), which now hands a `writeFork` completion to `foldFork` (fork.go) before releasing the latch — the Activate the resume queues goes out with that same pump, so the fold owns no second dispatch.
NOTES (2026-09-16): deviation — internal/tui/sessionsave_test.go joins Files: item 19's `TestQueuedForkWaitsForTheSaveAndCarriesTheChild` pinned "the parent stays active after the fork folds", which this item's completion-fold switch makes false by design; its tail now drives the child's Activate the fold queues and asserts the child active and the queue dry.
NOTES (2026-09-16): deviation — internal/tui/seam_test.go joins Files: `fakeEngine.CutSnapshot` records its drop counts (`cutCalls`) so the driver test can assert `CutSnapshot(1)` as the plan's Tests line requires.
NOTES (2026-09-16): the completion fold skips the switch on `m.eng.InExchange()` as well as `m.busy()` — a parent resumed mid-task is idle to the Model but its RestoreSession would be refused (agent.go's inExchange check) — with the same `forked as <id> — resume it from /sessions` note; the plan named only the busy case.
NOTES (2026-09-16): consequential edit — internal/tui/doc.go: made necessary by the new fork.go (the package map TestDocMapNamesEveryFile enforces).
NOTES (2026-09-16): consequential edit — internal/tui/command_test.go: made necessary by the new `fork` row (TestCommandTableDrivesParserAndMenu's verb enumeration).
