...
## 1. Domain: the sync classes take an argv handler, a gate decision and a `Generation.Sync` lane
...
## 4. Agent: the sync-lane argv executor, its permit row and the Driver reporter
...
## 5. Agent: the advise slot — spans collected at `post-tool-result`, fenced on the closing tool result
...
## 6. Agent: user argv advise through the executor, with the `file-changed` narrowing
