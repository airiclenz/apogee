...
## 7. Agent: the gate stage of the Approver at `pre-tool-exec` — ✅ DONE (2026-09-09)

NOTES (2026-09-09): `applyGates` drops the `tool string` parameter the item's signature names — it is `call.Tool`, and a second copy of one fact invites the two to disagree at the two call sites that pass it. The signature is `applyGates(ctx, turn int, call domain.ToolCall, verdict resolution) resolution`; the SeamPayload's `tool` comes from `call.Tool`.
NOTES (2026-09-09): the nil-Approver rule is reproduced in `gateAsk` rather than delegated to `finishGate` — `applyGates` is handed a finished verdict and not the `resolutionInput` `finishGate` reads, so calling it is not available. The branch builds the same fresh Refuse with `noApproverReason` and the same audit metadata, and is pinned by `TestGateAskWithNoApproverRefuses`.
NOTES (2026-09-09): the item's guard (a) — `fireLeg` skips class gate at every seam — replaces behaviour two existing tests pinned, so both are recast rather than left failing: `mechanism_dispatch_test.go`'s `TestBypassGate` gains an `onlyAt` term saying a gate dispatches at pre-tool-exec alone (through the gate stage) at every Bypass setting, and `reactions_test.go`'s `TestFireBypassMatrix` row "armed gate survives" becomes "an armed gate never reaches the cascade", with its Bypass survival now pinned by `TestGateDenyStillDeniesUnderBypass`. Both files join FILES.
NOTES (2026-09-09): the 5s gate class default is pinned on `syncTimeout`'s value (`TestGateClassDefaultDeadline`) rather than by a sleeping command, and the timeout BEHAVIOUR is pinned with an explicit 150ms `timeout:` — items 4 and 6's own rationale, since a literal class-default sleep would add five seconds to every suite run for a fact the table states exactly.
NOTES (2026-09-09): a gate that could not answer books TWO events, per the item's "a failure additionally reports per item 4": item 4's `failed` firing plus the operator line, and then the `ask` firing the escalation itself books.
NOTES (2026-09-09): consequential edit — internal/agent/doc.go: made necessary by adding gate.go (docmap.Check enumerates every file in the package). The map's "Thirty files" count needed no change: it read Thirty at 29 files, and gate.go makes it accurate.
...
## 9. Config: the `gate:` key ships; `advise:` stays refused — ✅ DONE (2026-09-09)

NOTES (2026-09-09): the item's Tests line asks `TestLoadFileConfigResolvesTheReactionsBlock` to resolve ONE entry carrying `run:` and `gate:` into two reactions (30s/5s, then two 2s). That entry cannot exist at this stage and the ratified call *Lanes in one list* is why: the `on:` list reaches every reaction the entry arms whole and each is validated, so an entry carrying both keys would need a Moment that is at once a notice (`run:`) and `pre-tool-exec` (`gate:`) — there is none, and either reaction would be refused. The two-key case becomes expressible only when `advise:` ships (item 13): `on: [file-changed]` is a notice AND an advise Moment, so `run:` + `advise:` resolves to two reactions of one id. The test therefore pins the same three facts across separate entries — a `gate:` entry resolving to one class-gate ArgvHandler reaction at the 5s class default, and a second taking `timeout: 2s` over that default — while the observe rows keep their 30s assertion. Nothing in the What was relaxed: the full `on:` list still reaches every reaction and every one is validated.
NOTES (2026-09-09): consequential edit — internal/config/options.go: made necessary by projectReactions now writing both lanes; the `Reactions` doc comment called the field "the resolved global user-origin observe list" and "the ONE lane".
NOTES (2026-09-09): consequential edit — internal/config/config.go: made necessary by `gate:` shipping; the `fileConfig.Reactions` doc comment said an entry takes "the one action ... under `run:`" and that nothing a Reaction does reaches the model, which is now scoped to the observe class.
NOTES (2026-09-09): consequential edit — internal/config/doc.go: made necessary by one entry resolving to several reactions; the package map called reactions.go "the on-disk shape of one user-origin observe Reaction".
NOTES (2026-09-09): the `KeyRegistry` "reactions" row still reads `Desc: "... observe-only, never seen by the model."` and counts `len(o.Reactions)` rather than distinct ids — item 10 owns that row (its regression guard names the `Read`), so both are left to it.
NOTES (2026-09-09): `handler`'s argv arm and the new `gate:` reader share one `argvList` decoder, so the two keys differ only in the sentence a wrong shape earns; the per-entry timeout parse moved into `classTimeout(id, fallback)`, which carries a spelled `0s` through unchanged so `reactions.Validate`'s non-positive-deadline refusal still sees it.
...
## 10. Driver: every wiring site splits the lanes, reports sync failures and reloads `Sync` — ✅ DONE (2026-09-09)

NOTES (2026-09-09): the item's "`lateEngine.SetReactions` compares Sync as it compares Observe" is delivered as the comparison's SCOPE rather than a second `reflect.DeepEqual` reach. A sync-lane difference has no consumer — the `moved` test exists only to keep a Replace off the Runner, and the Runner never fires a sync Reaction, so feeding Sync into it would retire an observe generation for an edit that did not touch it (the very cost the existing comment argues against), while computing a `syncMoved` nothing reads does not compile. The sync lane is therefore weighed by that same rule and lands on the other side of it: the doc comment states this explicitly, and `TestSetReactionsSkipsTheRunnerWhenObserveIsUnchanged/SyncOnlySwapReachesTheAgentAndNotTheRunner` pins it — a gate-only swap reaches the bound Agent's `Generation().Sync` and swaps the Runner zero times.
NOTES (2026-09-09): `liveSettings.setObserve` became `setReactionLanes(observe, sync)` as the item asks; its one test caller (`cmd/apogee/schedule_test.go:1326`) and the sentence naming the old setter in that file's doc comment moved with it.
NOTES (2026-09-09): `wire_settings.go`'s `next.Reactions` folds the lanes back observe-first, then sync — each lane keeps its file order, and `domain.SplitLanes` divides them again identically, but a file interleaving the two classes projects its entries regrouped by lane. The only reader of the order is the read-only `/settings` structured view of the block.
NOTES (2026-09-09): the "no `cmd/apogee` site sets `Config.Reactions`" guard (`TestNoWiringSiteWritesConfigReactions`) parses each non-test file and looks for a `Reactions:` composite-literal key rather than grepping the text: the new comments that state the rule contain the literal string, so a textual grep would fail on the prose that documents it.
NOTES (2026-09-09): the reporter reaches `firingConfig` through a new `firingInputs.report` field (nil ⇒ `Config.Report` stays nil, as a bare composition is), so each Driver hands one function to both its Runner and the engine.
...
## 15. Manual: `reactions.md` and `configuration.md` describe the two cells — ✅ DONE (2026-09-12)

NOTES (2026-09-12): the acceptance sweep grep (`observe-only`) matched the ADR 0073 link's filename (`0073-hooks-are-observe-only-…`) in `reactions.md`, so the intro's parenthetical now links ADR 0076 — the decision for the shipped state, which itself links 0073 — and names ADR 0073 in plain text as the `run:` posture's reasoning; no other way to satisfy the grep without renaming the ADR.
NOTES (2026-09-12): the manual's `## Where a failure shows` and `## Advising the model` both state that a sync-lane failure is reported each time it fails (`reportReaction` in `internal/agent/syncexec.go` carries no once-per-entry fold; that fold is the observe runner's) — stated as shipped, not changed.
NOTES (2026-09-12): the template comment's "off under bypass:" was rewrapped so `bypass:` never opens a comment line — `TestSpliceScalarSettingRoundTripsEveryEditableKey` reads a line-leading `# bypass:` as a second commented-out key.
NOTES (2026-09-12): retry on the verifier's FIX — the fence example in `reactions.md` ("3. The fence") and the `advise:` comment in the seeded `config.yaml` now read `at post-tool-result` for the `on: [file-changed]` entry, matching `adviceOf` (`internal/agent/reactions.go`, `Moment: m` at the `post-tool-result` seam) and the `TestE2EAdviseFileChangedFiresOnTheWriteAlone` trailer; both gained the clause that the header names the seam the advice was fenced at while the stdin document's `event` names the entry's Moment. `configuration.md` quotes no header, so it needed no change on this point.
...
## 16. Design docs: CONTEXT.md, the confinement contract's §10.4 row, the greenfield and ADR 0076 notes — ✅ DONE (2026-09-12)

NOTES (2026-09-12): consequential edit — CONTEXT.md: the `Reaction` entry's `Outcome` sentence (`{Retry, Inject, Defer, Edited, Detail}` and the retry/defer/intercept action list) was made stale by the shipped `Gate` field and the sync lane's `advise`/verdict/`failed` actions, so it names them; same entry, same file, no new sentence about reservation.
NOTES (2026-09-12): consequential edit — docs/design/reaction-core-greenfield.md: the header's "Implementation plan not yet written" was made false by the §9.4 `Plan C` line pointing at a plan, so it now points at §9.4, whose Plan A and Plan B lines gained links to their archived plan docs beside the new Plan C link.
NOTES (2026-09-12): the contract's original §10.4 paragraph said "Pre-request, pre-tool-exec, post-tool-result and history-rewrite get no permit" — rewritten as a two-row minting table (post-response via `hookExecutionCtx`, the sync lane via `syncPermitCtx`) so the Acceptance grep `get no permit` no longer matches; the phrase "carry no permit" now covers pre-request and history-rewrite only.
