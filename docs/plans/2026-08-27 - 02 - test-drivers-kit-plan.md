...
## 11. T-06 + T-10 + T-13 — usage with cached tokens, the forced-approval `Fix:` pane, key arming — ✅ DONE (2026-08-28)

NOTES (2026-08-27): the item expects the forced pane's `Fix:` continuation rows to "hang under the `Fix:` indent"; the approval pane has no per-field hanging indent — its body is prose the popup wraps to the body width, so the continuation rows start at the pane's body column, the same column `Reason:` and `Fix:` do. The assertion pins the actual geometry (every continuation row at the body column, nothing clipped, the sentence's last word on the last row) and the golden `t10-forced-pane.txt` records it. Checklist T-10's step 5 / fails-if reading ("wrapped flush-left") is therefore not settled by an assertion — see DEFER.
NOTES (2026-08-27): T-13 runs on the PTY driver rather than in-process, because the evidence its step 9 asks for is the `--tui-trace` file and `tui.Build` refuses a trace beside a driver-supplied output (item 4, amendment A1). Its "early key" is `holdKey` — the key written every 5 ms from the moment the prompt is sent until the pane is on screen, the driver's reading of the checklist's "hold the `a` key down".
NOTES (2026-08-27): the judge rubric carries T-10's Pass when / Fails if verbatim but is scoped by an `Extra` to the WORDING half only ("does the hint read as help beside an open question"), which is the judgment the item names; the pane's geometry and the auto/allow/deny halves are asserted cell by cell in the same file.
NOTES (2026-08-27): T-10's auto half asserts the rung from the footer's mode marker and cancels its pane instead of first running an ordinary terminal call as a contrast — a confined subprocess costs ≈ 5 s under `-race` and the contrast is already asserted in the ask-before control and in `internal/agent`'s ladder tests. It needs a 140-column terminal: the footer drops the mode marker when a long temp workspace path fills the row.
NOTES (2026-08-27): T-13 step 7 asserts that `j`/`k`/PgUp do not RULE on the pane, not that the viewport moved — the scripted conversation is short enough to fit the viewport, so a scroll assertion would fail on having nowhere to go.
NOTES (2026-08-27): `e2eSession.RelaunchWith` was added to `cmd/apogee/e2e_support_test.go` (outside the item's Files line) because `--continue` names the record the FIRST run wrote and so cannot be passed at the launch that creates it; documented in `docs/design/test-drivers.md`.
NOTES (2026-08-27): `cached-usage.yaml` is hand-written, as the item says; the design doc's "Recording a fixture" section now names it as one of the two fixtures that are, and states that a re-recording against a prefix-caching server changes the fixture and the test's expectations together.
NOTES (2026-08-27): measured — `go test -race -count=1 -run 'TestE2EUsage|TestE2EApproval' ./cmd/apogee/` = 11.1 s (the five driven tests: 1.98 + 1.33 + 1.24 + 4.05 + 0.90 s); the whole package is 54.6 s under `-race`, which is past the plan's ≤ 15 s e2e budget across items 9–11 and is item 16's to reconcile.
NOTES (2026-08-27): observed while verifying — `go test -race ./...` intermittently fails `internal/tuitest`'s own `TestPTYDriver*` and `TestCheckLeaks*` with "goroutine(s) outlived the test by 2s" (bubbletea's `readLoop` parked in `ultraviolet.TerminalReader.StreamEvents`). Reproduced at BASELINE with this item's changes stashed (2 of 3 runs), so it is item 6's `leakGrace = 2s` being too tight when `go test ./...` runs packages in parallel, not this item's doing. Every targeted run of the package passes; see FOLLOW-UP.
...
## 12. T-12 + T-20 — hostile text on every surface; `[✔]`, wide-rune width authority across a scheme switch — ✅ DONE (2026-08-28)

NOTES (2026-08-28): T-12 steps 8–9 are asserted on the tool RESULTS the model was handed rather than on the transcript. `list_dir`, `find_files` and `grep` declare no `body` in the tool registry (`internal/tui/toolregistry.go`), so a listing's rows exist nowhere on the screen — the transcript paints the call's target and its outcome slot ("6 entries") and nothing else. `TestE2EHostileToolResultsKeepOneRowPerEntry` counts the rows out of the stub's request log and pins the literal `\n` / `\r` spellings (`escapeRowBreaks`), with the frame's outcome slot beside it; the file header says so.
NOTES (2026-08-28): those tool results keep RAW ESC and bidi-override bytes — only row breaks are escaped — so no assertion claims otherwise. A tool result is text for the model, not a rendering surface; every surface T-12 names (the probe report, the footer, the `/skills` note, the panes) is asserted separately and each one is inert.
NOTES (2026-08-28): T-12 step 3 finding, reported as the checklist asks rather than failed — the footer's WORKDIR segment is NOT escape-stripped. `Model.footerContent` (`internal/tui/model.go`) puts the host, the model id and the effort default through `stripEscapes` and appends `m.workdir` raw, so a workspace root whose name carries `ESC [31m` paints the terminal red. The test logs it (`reportWorkdirCell`) and `assertNoLeakedColour` exempts the footer row for that one reason; every other row of every frame is held to "no leaked colour". See DEFER.
NOTES (2026-08-28): T-12 step 6 does not describe the surface. The `mode` value sub-list TRUNCATES its right-hand column with an ellipsis at sixty columns instead of wrapping — `renderSettingsSubList` (`internal/tui/settings.go`) sets `menuRows` without `wrapRows` — so there is no continuation row to check the indent of. Asserted instead: the `auto` sentence starts in the same column as the `(current)` cell, is clipped inside the pane's own border, and authors no second row. The item's "wrapped popup body" claim is settled by step 10's approval pane, which does wrap, and by the golden `t12-pane-60.txt`.
NOTES (2026-08-28): the hostile fixture root is `os.MkdirTemp("/tmp", "apogee-hostile-")` — the checklist's own `/tmp/apogee-hostile-…` — rather than `t.TempDir()`. The `/skills` golden prints skill PATHS and a row of that note has to fit at a hundred columns; a `t.TempDir()` path carries the test's name and a counter, and on macOS TMPDIR alone is fifty columns. It is still a throwaway directory the test creates and removes on cleanup, which is what the "no test touches the real home" rule asks.
NOTES (2026-08-28): `launchTUIIn` was added to `cmd/apogee/e2e_support_test.go` (outside the item's Files line) because the fixture's escape is in the workspace ROOT's own name and a root is named when it is created — no launch helper could take that workspace after the fact. `launchTUIConfigured` is now one line over it.
NOTES (2026-08-28): T-20's terminal is put into Unicode-core mode before the launch (`unicodeCoreTerminal` writes `ESC [?2027h` into the `Screen`). `x/vt` answers the DECRQM 2027 query "not recognized" by default, which leaves apogee's width authority on `ansi.WcWidth` for the whole run — and the checklist's own preconditions say such a terminal cannot regress step 5 and must be recorded as not covered. The item's "assert the diag log shows the query was answered 2/1" is therefore `mode 2027: 1 (set)` plus `width-method: grapheme`, read from `--tui-diag` and asserted BEFORE any column claim.
NOTES (2026-08-28): `widths.yaml`'s table makes the VS16 cluster the WIDEST cell of its column, three of them over. A column is as wide as its widest cell, so a table whose widest cell measures the same under both measures lands in the same columns either way and could not fail. Measured with the fixture as written: crossings 9,32 and rule end 52 under Unicode core against 9,29 and 49 under wcwidth — a reset authority moves the layout three columns.
NOTES (2026-08-28): the goldens need three redactions beyond the session's own — the footer's tail past the model cell, the fixture root (and its `EvalSymlinks` form, since macOS resolves /tmp into /private), and the padding between a redacted path and the scroll rail at the end of its row, whose column is downstream of the path length the redaction before it just changed. Only path-bearing rows are touched, so the rest of the frame keeps the alignment it was painted with.
NOTES (2026-08-28): `WaitQuiet` immediately after a keypress is not a wait for that key to land — the screen has been quiet since before the key was sent, so the check passes at once on a frame the program has not answered. It cost the first ␣-toggle assertion (three presses, no repaint, nothing ticked). `pressAndRepaint` waits for the paint the key caused; the design doc's "Gates and budgets" now states the rule beside the settle rule it is a corollary of.
NOTES (2026-08-28): `settingsCursor` scans a frame top-down, which is right only for the full-height key list — nothing else is on the screen then. A short value sub-list leaves the transcript visible above it and a prompt row starts with the very same ❯, so the scheme walk reads its cursor bottom-up (`paneCursor`).
NOTES (2026-08-28): both hostile tests skip on Windows — the fixture's names hold an ESC, a newline and a carriage return, and Windows has no such files.
NOTES (2026-08-28): measured — `go test -race -count=1 -run 'TestE2EHostile|TestE2EWidth|TestJudgeHostile' ./cmd/apogee/` = 12.1 s across six driven launches (0.01 + 0.65 + 1.91 + 2.54 + 1.49 + 3.91 s); the settings pane is most of it, walked one arrow at a time, and the width item opens it twice (T-20 step 7's switch back). The whole package under `-race` is now 65.4 s against the plan's 5.5 s baseline, still item 16's to reconcile as recorded under item 11.
...
## 13. T-15 + T-16 — outcome-slot tone and elision; live state follows the running session — ✅ DONE (2026-08-28)

NOTES (2026-08-28): T-15 steps 3–4 are asserted on the REOPENED session, not on the live one. A live Esc discards the interrupted Exchange (`Model.foldCancelled`) and closes nothing: the delegation's call is left OPEN — its leader dots run to the row's edge with no verdict in the slot at all, and the block keeps the ✦ that marks work still standing behind it. The red `interrupted — the run did not finish` the checklist asks to see is written by the replay that closes every call a record left open (`closeInterruptedCalls`), so it exists from the reopen onwards and nowhere before it. The live half is asserted for what it does say (the `cancelled` note, no ✓); see DEFER.
NOTES (2026-08-28): T-15 step 5's `task:` lead is not reachable from a cancelled delegation either, for a second reason: a block whose collapsed paint hides nothing is marked `targetNone` (`renderToolBlock`), so an interrupted run with no span is not a click surface at all — ⌥↑ does not stop on it and ⏎ cannot open it, which puts the prompt it carried out of reach. Step 5 is therefore asserted on a delegation that FAILED — its child's first request refused with HTTP 500 — which has an answer behind it, so the prompt stands at the top of its rail above that answer (the checklist's own second sentence for step 5). The unframed `task: ` shape is unit-covered in `internal/tui/subagentblock_test.go`.
NOTES (2026-08-28): the item and the checklist say Shift-Tab THREE times reaches Auto; on today's ladder it is two (ask-before → allow-edits → auto → plan). `reachRung` walks by the footer's marker rather than by a count of presses, so a reordering that breaks nothing does not break this test.
NOTES (2026-08-28): `configWatchTiming` is a `watchTiming{Interval, Settle}` pair applied to the watcher before `Start`, not the `filewatch.Options{}` the item's text names — that type does not exist. `filewatch.Watcher` exports `Interval` and `Settle` as fields settable before `Start` (filewatch.go:67-73) and documents them for exactly this use, so the seam carries the two durations to them and a zero field leaves the package default in place.
NOTES (2026-08-28): T-16 step 12's "more than one block live at once" is not observable under stubllm and is asserted as "all three delegates ran against the new server, and all three blocks appear". The fan-out width resolves from the bound entry's `parallel-agents:` pin and the server's own `total_slots` (`parallelAgentsCap`); a launch-profile move builds an entry with no pin, and stubllm answers no `/props` by design (item 2), so the cap is 1 whatever the move did. The re-follow itself is unit-covered in `upstream_test.go`, as the checklist's own Part line says. See DEFER.
NOTES (2026-08-28): `launchTUIOn` was added to `cmd/apogee/e2e_support_test.go` beyond the item's "launchTUI sets the watch timing" — `llama-launcher:` sits INSIDE the `servers:` entry (ADR 0029 D4) and no line appended to the file afterwards can reach in there, so the launcher half needs a home the caller wrote. `launchTUIIn` and it now share `e2eGuards` (leak check, ambient-config refusal, fast watcher) and `startSession`; every existing call site keeps its signature and its cleanup order. Documented in `docs/design/test-drivers.md`.
NOTES (2026-08-28): the `/model` load is driven through a `gatedLauncher` — the bridge's own `fakeLauncher` with `loadProfile` held open until the test releases it. A real profile load takes seconds to minutes (the facade's contract) and the footer's `loading <name>…` readout only exists while it runs, so a fake that returned at once would make step 11's own frame unobservable rather than fast.
NOTES (2026-08-28): this file walks the settings pane DOWNWARDS (`settingsGoDown`) rather than through the shared `settingsGoTo`, which walks up because item 12's rows sit past the middle of the registry. T-16's rows sit near the top; walking down cost 3.7 s less of the package's budget and reaches the same rows.
NOTES (2026-08-28): no judge rubric in this item, and none is asked for — T-15's judgment half is which CELLS carry the error tone and where the elision rule is drawn, both of which the emulator settles outright.
NOTES (2026-08-28): measured — `go test -race -count=1 -run 'TestE2EOutcome|TestE2ELiveState' ./cmd/apogee/` = 22.0 s across seven launches (8.34 + 2.55 + 7.00 + 1.27 + 0.98 s of test time); the whole package under `-race` is now 85.4 s against the plan's 5.5 s baseline, still item 16's to reconcile as recorded under items 11 and 12.
...
## 16. Closeout — checklist reconciliation, ISSUES.md residue, design-doc final, `make check` — ✅ DONE (2026-08-28)

NOTES (2026-08-28): the CHANGELOG entry above is the ONE consolidated Changed/Added entry the item asks for over plan items 9–15, and it is meant to REPLACE the individual `## CHANGELOG` sections of sidecars `notes-9.md` … `notes-15.md` — applying both would duplicate the same work. Item 8's kit entry (`notes-8.md`) stays as its own entry, per the item's text.

NOTES (2026-08-28): the item names `CHANGELOG.md` in its Files list, but the run's own protocol forbids an implementer from editing the CHANGELOG — the entry text goes in this sidecar and the closeout applies it. Followed the protocol; the file is untouched.

NOTES (2026-08-28): the item's "the 'Not observable' rows are exactly: font tofu (T-20), felt flicker (T-24), real desktop applications (T-19), `brew upgrade` … (T-21/T-23)" was NOT applied literally. The design doc's table has fifteen non-empty cells in that column, and ten of them name the instrument that asserts the claim INSTEAD of that row's driver (the session record, the stub's request log, `internal/tools/network_funnel_test.go`, `upstream_test.go`) — accurate documentation that items 9–15 wrote deliberately. Deleting ten true cells to satisfy a count would lose real information, so instead a paragraph above the table now splits the column explicitly: those ten are pointers, and only the four named above are irreducible. Those four, and only those four, are what `ISSUES.md` records.

NOTES (2026-08-28): the item's budget remedy — "if the set exceeds 15 s, the slowest test is moved behind `APOGEE_E2E_SLOW=1`" — was NOT applied. Measured: 121.7 s under `-race` for 36 tests. The remedy cannot reach its own target (gating the slowest, `TestE2EEgressLongStreamIsNotDeadlined` at 25.7 s, still leaves ~96 s), it contradicts the rule items 4–8 landed in this same document ("no build tags and no `-short`: a test that only runs when someone remembers a flag is a test nobody runs"), and it would delete the only observation that the provider client sets no response-wide timeout — the twenty-five seconds ARE that test's claim, as item 14 documented. The 15 s figure predates items 9–15, each of which landed and had its own measured set (≈ 9.5–25 s) verified. What was done instead: the design doc's budget section now carries the measured per-file table as the authority, and states plainly that nothing was gated and why. Reversible in one commit if the owner wants the gate.

NOTES (2026-08-28): `ISSUES.md`'s "The `ResidualNotice` prints have no test" was narrowed rather than removed — item 15's `landlock-abi-1-2` CI job now proves the HEADLESS disclosure reaches stderr on an ABI 1–2 kernel, but `cmd/apogee/wire_boot.go:290` (TUI boot) and `cmd/apogee/daemon.go:230` are still exercised in neither direction. No other ISSUES.md line was closed by this plan: the fail-closed proxy paths (`internal/tools/network.go:338`, `internal/mcp/transport.go:201`) and `/server`'s `switchServer` cap follow (`wire_verbs.go:132`) were both checked against the new tests and remain uncovered.

NOTES (2026-08-28): checklist record mode applied per the `test-checklist` skill's own `prompts/record.md` — sixteen items ticked `- [x] Pass` with a `**Result — automated, 2026-08-28**` block naming the tests, a `## Manual results log` section added before `## Appendix`, and the `**Status:**` line given its `(16/19 recorded)` count. T-03, T-04 and T-06 got their automation recorded in an `**Automated since**` block but their boxes were left UNTICKED — the item says those owner-tested boxes stay the owner's.

NOTES (2026-08-28): DEVIATION, owner-authorized — this item amends item 5/6's `internal/tuitest` to fix the goroutine leak that was holding `make check` red, rather than deferring it. Root cause: bubbletea's input read loop parks in its cancel reader's `EpollWait`, and a KILLED program closes that reader without waiting for it (`shutdown(kill)` skips `waitForReadLoop`, tea.go:1249-1255). Closing an epoll descriptor does not wake the `EpollWait` already parked on it, so a loop still parked when the reader closes is parked for the life of the process. `Driver.Kill` already ended the input first to give the loop an EOF to leave on, but never waited for it to actually leave. Fix: the pipe's read end is now a `driverInput` — an embedded `*os.File`, which the epoll cancel reader still requires — that records the read returning an error, and `Driver.Close`/`Driver.Kill` call the new `Driver.joinReadLoop` between ending the input and releasing (or cancelling) it. A run already returned, or an input no loop ever read, needs no wait; a 5 s backstop covers a program that never started. `Driver.ended` was added so that wait cannot consume the buffered result `Done()` carries. Measured: the reproducer (8–12 busy loops on 9 cores) went from red every run to 0 leak reports in 5 runs and 2 further `-count=3` runs; the package's own time fell 9.3 s → 2.2 s, since each report had been costing a 2 s `leakGrace`. `CheckLeaks(t)` was added to the six in-process driver tests so the regression fails where it is caused rather than in whichever later test scans next — verified by reverting the join, which turns all six red. `leakGrace` and `t.Parallel` are untouched.

NOTES (2026-08-28): DEVIATION — a SECOND defect stood between this item and a green `make check`, and was fixed on the same authority. `TestRecorderReplaysWhatItRecorded` (item 1/2's `internal/stubllm`) failed 2 runs in 10: a streaming client stops at the `[DONE]` event and returns to its caller while the recording proxy is still one `Read` short of the EOF that files the turn, so a `Recorder.Close` called immediately afterwards wrote a fixture short its last turn — silently. Fix in `internal/stubllm/record.go`: the recorder counts requests in flight, `finish` and the proxy's error handler settle them exactly once (`capture.settled`), and `Close` waits for the count to reach zero with a 2 s backstop. Confirmed by probe (a 200 ms sleep before `Close` removed the failure) and by 20 clean runs of the recorder tests where the baseline failed 2 in 10. The recorder section of `docs/design/test-drivers.md` gained the paragraph explaining the wait.

NOTES (2026-08-28): `make check` now exits 0 — verified three times end to end, the last after restoring an incidental `go.sum` churn that a single-package `go vet` had added (the file is untouched in the final tree). `cmd/apogee` 125.9 s, `internal/tui` 100.9 s, `internal/tuitest` 2.2 s, `internal/stubllm` 1.5 s; `gofmt`, `go vet`, `go build`, `scripts/check-pins.sh`, `actionlint`, the ADR-0010 invariant, the six-target cross-build and `--help` all pass.
