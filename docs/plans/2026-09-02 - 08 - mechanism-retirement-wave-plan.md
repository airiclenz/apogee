...
## 19. Empty-catalogue invariants: tests, the doc map and the `/settings` list — ✅ DONE (2026-09-03)

NOTES (2026-09-03): the item's Files list is partly already-done work: `internal/mechanisms/doc.go`, `benchreadiness_test.go` (the `:74` arm and the leave-one-out), `cmd/apogee/naming_test.go` and `cmd/apogee/wire_firing_test.go` were restated for the empty catalogue by items 14–17 and needed no further edit here — verified by reading each named site.

NOTES (2026-09-03): the leave-one-out arm at `benchreadiness_test.go:605` was left as item 17 shaped it — a LOUD `t.Skip` naming ADR 0071 rather than a deletion — so the idiom revives by itself the day a bench Driver registers a lab row. Deviation from the item's literal "leave-one-out drops".

NOTES (2026-09-03): `renderSettingsSubList` gained a `choices` parameter (the count of things a ⏎ could TAKE, which the enum's rows and the Mechanism list's toggles agree on until the list paints PROSE). Without it the one prose row of an empty catalogue would be highlighted as if it were a switch. The enum caller passes `len(values)` and is otherwise untouched.

NOTES (2026-09-03): the two red `cmd/apogee` tests the earlier items handed on were recast, not retired. `TestListMechanismsReadsTheBlock` → `TestListMechanismsOffersTheCatalogueNotTheBlock`: the list is `mechanisms.KnownIDs()` verbatim (empty, non-nil) and a `mechanisms:` block naming a RETIRED id adds no row — the same claim as before, keyed off the catalogue so it grows with a lab row. `TestScheduleFiringRunsAgainstTheCurrentBinding` swapped its observable from `spec.Config.EnableMechanisms` to the Validated-set NOTICE `firingConfig` composes: with the catalogue empty, an APPLIED and a merely OFFERED set arm the same nothing, so the roster stopped being an observable, but the two rungs still speak different lines. Keyed on the bound endpoint the probe record is found, the identity is medium-confidence, and the entry reaches the applying rung, which says it weighed the entry and could not use it (`skipping validated-set entry "bound-model"`); keyed on the launch snapshot the record is missed and the same entry earns the low-confidence OFFER line. The test asserts the first is present and the second absent. It reads them at `firingConfig` itself, from `w.live.firingSources(w.binding())`, because `fire` drops the composer's notices. The probe-record + user-dir-entry fixture stays — it is what a user upgrading into this wave holds, and the Firing must run over it rather than fail on it — but its set moved from the retired `syntax` to `a-row-no-build-carries`: a wholly-retired set is shed to empty and lands on `setRetired` from BOTH rungs, which tells them apart not at all, whereas an unknown member survives the shed and separates them. Nothing but the notice can: with an empty catalogue no set validates, so no member reaches the roster either way.

NOTES (2026-09-03): `TestPromotedRowsCarryTheirFloorGuardKey`'s table was short by THREE, not the two the dispatch named — `tool_result_cap` was missing too. All six promoted rows are now in it.

NOTES (2026-09-03): findings handed to this item that needed no code. `TestOffRampFloorIsEmptyWithNoCapOffRampRow`'s claim ("no catalogued row declares CapOffRamp") is now SUBSUMED by `TestProductionCatalogueIsEmpty` — with no row at all there is no declarer, and the off-ramp floor plumbing itself went at item 6, so re-pinning the old assertion would state a rule the design no longer has (a lab row may legitimately declare `CapOffRamp`). `TestListMechanismsAppliesTheOffRampFloor` / `…NilAndEmptyBuildTheOffRampFloor` / `TestDepsNeeded` / `TestEnableMechanisms_NonLibraryArmIgnoresLibraryDir` no longer exist — deleted with the machinery they asserted over. `TestRebindRebuildsMechanismsForNewModel` was already recast (item 17) onto the registry INSTANCE plus an unknown-ID refusal, which is the strongest proof an empty catalogue leaves. `Example_cataloguedMechanisms` keeps its early return: it is godoc for the plan-around-a-row idiom, and an Example driven only through the public surface has no synthetic row to make it non-vacuous.

NOTES (2026-09-03): `TestShipped_PinnedAgainstCatalogue` (internal/validated) still walks an empty roster and is vacuous by design — item 18 flagged that deliberately and it stays as the drift tripwire item 2 asked for. Not touched here.

NOTES (2026-09-03): the earlier attempt's replacement assertion in `TestScheduleFiringRunsAgainstTheCurrentBinding` (`spec.Config.Endpoint == url`) could not fail — `Config.Endpoint` is assigned from `in.entry.Endpoint` at `wire_firing.go:221`, a different statement from the resolution's key at `:145`, and a wrong `:221` makes the run error or hang before the assertion is reached. Deleting `:145` left the whole suite green. The notice assertion above replaces it and was mutation-checked twice against `wire_firing.go:145`: DELETING the line and CORRUPTING it to a literal endpoint each fail the test in ~0.01s with the offer line in hand, no hang. The runner-seam override that captured the endpoint is gone with it, and the test's doc comment no longer claims the run's own Config as the resolution's observable.
