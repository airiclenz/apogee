...
## 9. TUI: git_diff_range recovers regions — ✅ DONE (2026-08-19)

NOTES (2026-08-19): the recovery is reached through item 8's registry hook (`toolPresenter.regions`) rather than through the `body:` hook the item's text asks for — the dispatch's binding context, and the same reason item 8 recorded: a body hook returns lines only, and this item must also set `tv.Regions` for the split reading to compose at paint time. No `body:` hook was added at all: `detail: outputDetail` already IS this tool's floor, so the "falls back to today's outputDetail rendering for the WHOLE body" rule is what the untouched floor does when no section comes back.
NOTES (2026-08-19): the hook now answers in FILE SECTIONS (`[]diffFileRegions`) instead of a flat `[]domain.EditRegion`, and `toolView` grew `RegionFiles` — the file each region was cut from, one entry per region and aligned index-for-index with `Regions`, so the change itself keeps one home and the names annotate it (`regionFileSections` regroups the two for either reading). `viewDiffRegions` returns one NAMELESS section and paints exactly as before. Without this the split reading, which is composed at paint time, had no way to know where one file's regions end and the next file's begin.
NOTES (2026-08-19): the item's Files list names only `toolpresent.go` and its test, but four more files had to be edited. `toolbranch.go`: `splitBody` — the shared decision seam item 7 landed — composes the panes section by section and paints the muted header row, which is the item's own "regions and headers stored so item 7's composer paints them per file section"; its test file gained the paint pin (`TestSplitDiffPaintsAFileHeaderPerSection`, both widths). `internal/tui/doc.go`: its registry narration named `viewDiffRegions` alone as the recovery, which stopped being the whole story. `docs/layout/split-diff-layout.md` (the canonical row-level layout spec, an authoritative source of this plan): it had no file-section rule at all, and its gutter rule said "the widest number shown in that body" where a multi-file body sizes one gutter per file section. No other file touched.
NOTES (2026-08-19): "never a half-parsed mix" is drawn one step wider than the item's literal "any line the parser does not recognize" — a `diff --git` section that parses cleanly but yields NO region (a rename, a mode change, a binary file) also fails the whole body, because dropping it would paint a diff of fewer files than the tool printed. This is what makes the item's "binary-file and rename-only sections fall back to plain rendering" and its "a malformed body falls back wholesale" the same rule rather than two.
NOTES (2026-08-19): a muted header row is painted for EVERY file section, not only when the diff spans several files: `git_diff_range`'s target is the ref range, so unlike `view_diff` the block names no file anywhere else.
NOTES (2026-08-19): item 10 (codec, not yet done) persists `toolView.Regions`; `RegionFiles` is display state of exactly the same class and needs the same additive field, or a decoded multi-file git diff loses its header rows in the SPLIT reading only — `Details` carries them, so the stacked reading survives a reload either way. `regionFileSections` already degrades to one nameless section when the names are missing, so the loss is a missing header and never a wrong body.
