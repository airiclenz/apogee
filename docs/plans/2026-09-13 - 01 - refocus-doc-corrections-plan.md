...
## 2. CONTEXT.md — the three Bypass sentences name the context-fill notice — ✅ DONE (2026-09-14)
...
## 3. ADR 0076 A8 — `SetReactions` is `Runner.Replace`'s only caller — ✅ DONE (2026-09-14)
NOTES (2026-09-14): retry — the prior attempt's ADR and greenfield rewrites were kept; only the A8 paragraph was re-flowed to the ADR's ~100-column width (line 310 had been left at 156 columns), with "becomes the only caller of `Runner.Replace`" kept on one line for the Acceptance grep.
