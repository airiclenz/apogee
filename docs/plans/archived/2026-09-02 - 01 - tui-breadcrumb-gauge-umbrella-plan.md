...
## 3. Fold a same-type run of 2+ calls under the `✦ Tools (N calls)` umbrella — ✅ DONE (2026-09-02)

NOTES (2026-09-02): `runAggregate`'s new identical-summary rule is implemented as the helper `sameSummary`, which returns the FIRST member's `branchSummary` whole rather than rewording it — the marks it carries (quoted, the verdicts) are already true of the text every member shares, and it matches how a run of one is already returned verbatim.
NOTES (2026-09-02): the item names the aggregate case `TestRunAggregate/"a stat with no arithmetic is blank"` as "rewritten to want `exit 0`"; the subtest was also RENAMED ("a stat with no arithmetic the run agrees on is shown once") because the old name states the opposite of the new want. The `PASS`/`FAIL` case was renamed for the same reason ("a verdict the run disagrees on is blank"); its want is unchanged.
NOTES (2026-09-02): guard (d)'s grep rule was applied by running the package rather than by a name list — every test that asserted member rows for a same-label run failed and was moved one level down under an opened type row. Tests touched beyond the ones the item names: `TestGroupHeaderCountIsFaintAndInert`, `TestGroupMemberKeepsItsSummaryAndClipsTheTarget`, `TestGroupMemberMarksNameTheirOwnCalls`, `TestExpandedGroupMemberPaintsTheSketchShape`, `TestExpandedMemberGutterIsNotTheSubAgentRail`, `TestExpandedBlockLiftsItsDetailTone`, `TestRenderGroupWithInFlightMember`, `TestRenderSingleCallSharesTheGroupShape`, `TestRenderGroupsInsideSubAgent`, `TestRenderGroupsDifferentToolsSharingALabel`, `TestRenderGroupBreakers`, `TestAnsweredAskUserBlocksNeverGroup`, `TestTranscriptCodecReDerivesAnsweredQuestionSolo`, `TestBlockMarksAgreeWithTheMouseMapping`, `TestBlockCursorEntersAtTheEndItsKeyPointsAwayFrom`, `TestLiveBlockHeaderStarBlinks` (its two "a group …" cases).
NOTES (2026-09-02): `TestRenderMarksTheWholeBlock/"a body-less group is no target at all"` was renamed to "a body-less run marks its type row and nothing else" — the item says it expects `targetType` on the type row, which the old name contradicts.
NOTES (2026-09-02): under an open type row a member's gutter follows its type row's and the two are painted as ONE style run, so the three gutter-tone assertions (`TestExpandedMemberGutterIsNotTheSubAgentRail`, `TestExpandedBlockLiftsItsDetailTone`, `TestSplitDiffPaintsUnderAnOpenMembersGutter`) assert `memberGutter + glyphMemberGutter + " "` rather than `memberGutter`. Likewise the type row paints its count in one run with the space before it, so `TestGroupHeaderCountIsFaintAndInert` asserts `th.toolIndicator.Render(" (2)")`.
NOTES (2026-09-02): guard (e)'s `mouse_test.go` subtest resolved to the first branch it offers — with a member open the umbrella header is `targetUmbrella` and a click there closes both levels; that is what the subtest now asserts (a new `typeRow` helper finds the row).
NOTES (2026-09-02): consequential edit — internal/tui/paintcache.go: made necessary by the umbrella no longer requiring runs of different tools (the `shapeToolSuper` comment said "adjacent runs of different tools").
NOTES (2026-09-02): consequential edit — internal/tui/toolblock.go: made necessary by the same change (`renderSuperGroup`'s doc comment opened "adjacent runs of DIFFERENT tools folded under one header").
NOTES (2026-09-02): `cmd/apogee/e2e_subagent_view_test.go` is in the item's Files list but needed no edit — only its golden was re-recorded, and the diff is exactly the one the item predicts (`✦ Read (2)` + two member rows → `✦ Tools (2 calls)` + a collapsed `┕ Read (2) ⋯ 4 lines ▶`, one row shorter).
NOTES (2026-09-02): the prose in `internal/tui/doc.go`, `layout.md` and `docs/layout/tool-layout.md` still describes the same-type group shape; item 4 owns that rewrite and is not yet done.
...
## 4. Retire the same-type group shape and update the canon — ✅ DONE (2026-09-02)

NOTES (2026-09-02): `renderToolBlock` narrowed by taking a single `toolView` (the plan's first option) rather than a `len(views) == 1` guard — the impossible state is then unrepresentable. Its six call sites drop their `[]toolView{…}` wrapper; `guardPromotions`/`blockHidesWhenCollapsed` keep their slice parameters (the existing `subagentblock.go` idiom).
NOTES (2026-09-02): `TestPaintedTabBearingToolTargetKeepsItsColumn` re-pointed at single-view blocks — it now paints one block per view and keeps the same per-row `width - groupIndicatorCells(th)` column assertion (the plan's "single-view row" option).
NOTES (2026-09-02): consequential edit — internal/tui/toolbranch.go: made necessary by the deletion of renderToolGroup (the `toolCallRun`/`groupable` doc lines named the same-label group as a shape).
NOTES (2026-09-02): consequential edit — internal/tui/blocktarget.go: made necessary by the deletion of renderToolGroup (targetHeader doc named it as the painter of member rows).
NOTES (2026-09-02): consequential edit — internal/tui/subagentblock.go: made necessary by the deletion of renderToolGroup (`renderSubAgentGroup` compared itself to that list).
NOTES (2026-09-02): consequential edit — internal/tui/transcript.go: made necessary by the deletion of the same-label group (`toolRun`'s doc called it one of "both folded shapes").
NOTES (2026-09-02): consequential edit — internal/tui/mouse_test.go: made necessary by the deletion of the same-label group (two comments described calls folding into "one group" with no clickable header).
NOTES (2026-09-02): the three `(different types / super-group)` sketch headings in `docs/layout/tool-layout.md` were renamed to `(super-group)` — the plan's prose rule covers "names the mixed-type condition for an umbrella", and the umbrella now covers a single run too; the Go reference to that heading in `renderSuperGroup`'s doc follows.
NOTES (2026-09-02): `toolCallRun` (`toolbranch.go`) now has no production caller — it survives only in four tests that assert run boundaries. Not deleted: the item's deletion list is explicit and does not name it. See DEFER.
NOTES (2026-09-02): the tool-call display entry was removed from `IDEAS.md` as the item requires, but `IDEAS.md` is gitignored (`.gitignore:12`) and is therefore NOT in the FILES list — do not `git add` it.
