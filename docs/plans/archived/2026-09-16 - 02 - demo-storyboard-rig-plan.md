...
## 5. `demorig render` — storyboard-driven GIF; retire `render.sh` — ✅ DONE (2026-09-16)

NOTES (2026-09-16): `Build` takes a third argument, `source Size` (the ffprobed take geometry), beyond the plan's `Build(segments, frame)` — the fixed-size `crop=<src_w>:<src_h>` and the region centre must be literal source pixels and `Frame` carries no height; `segmentsFrom` (beats sorted by resolved time → segments) sits beside it in filtergraph.go.
NOTES (2026-09-16): an empty segment (a beat resolved on the take's last frame, hero beat 8 at `{video: end}`) contributes no piece rather than a zero-length `trim`; `Build` still rejects backwards, negative, unsorted and overlapping segments and a graph in which every segment is cut or empty.
NOTES (2026-09-16): a zoom is applied only to the pieces its `in`+`hold`+`out` span overlaps (hero beat 5: the 4s hold piece, not the 1.5× remainder), and the ramp is `1+(factor−1)*clip(min(rise,fall),0,1)` with a zero-length `in`/`out` written as a `gte`/`lt` step so no expression divides by zero.
NOTES (2026-09-16): `cut` is exercised by its own `Build` test rather than the golden — the hero storyboard has no cut beat and the golden stays the hero storyboard verbatim over fixed beat times.
NOTES (2026-09-16): the `renderTools` seam (probe + first-paint painter) is what `--dry-run` fakes in the test; the encode (ffmpeg, gifsicle) runs directly since a dry run never reaches it. `--dry-run` still probes the take, as the plan's fake-prober test implies.
NOTES (2026-09-16): consequential edit — cmd/demorig/main.go: made necessary by the new subcommand (registered `newRenderCommand(ffmpegTools{})`; the package doc's usage block gains the `render` line and the sentence deferring the render subcommand is replaced).
NOTES (2026-09-16): record.sh prints the render line after `demorig check` returns (inside the storyboard branch, so under `set -e` only a passing take gets it); a clip without a storyboard is not rendered, since the render reads its framing from one.
