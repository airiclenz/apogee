...
## 2. `domain.ContextCost` and `Agent.ContextCost()` — ✅ DONE (2026-09-17)

NOTES (2026-09-17): the ride-along-gated walk of the standingBlocks table is now one method, `standingRenders` (standingblocks.go), which `standingSystem` (loop.go) joins over and `ContextCost` counts over — the two producers share one walk rather than each restating the gate; `standingSystem`'s output is byte-identical (contextfiles/standingblocks/delegatereport tests green). Both files are outside the item's Files list; they are the read-first anchors and the edit is the item's own "agree by construction" made literal.
NOTES (2026-09-17): consequential edit — example_test.go: made necessary by the two new facade aliases in apogee.go (the file enumerates every exported facade type).
NOTES (2026-09-17): `ContextCost.Bytes` is the sum of the rows and `Tokens` the one estimate over that sum (not the sum of per-row estimates, which each round up); the standing joiners are not in the total, as PromptChars omits them too — the plan left the total to the producer.
...
## 4. Headless: the context cost and the measured Turn-1 tokens on run_finished — ✅ DONE (2026-09-17)

NOTES (2026-09-17): `Result.ContextCost` is taken after the delegation setters (SetDelegationSeat feeds the orientation block) and immediately before `Submit`, not beside `ContextFilesReport()` — still idle, before the first Step; it also rides the submit-failure exit like `ContextFiles` does.
NOTES (2026-09-17): the two Turn-1 keys are appended after `sub_agents` in the frame (key order `…, context_files, context_cost, undo_note, saved, usage, sub_agents, turn1_prompt_tokens, turn1_cached_prompt_tokens`); the plan fixed no key position.
NOTES (2026-09-17): the "Turn-0 maintenance call's cumulative figure" half of the Turn-1 test is pinned at the tap level (`TestEventTapTurn1IgnoresAMaintenanceReading`) rather than over stubllm — stubllm cannot script an engine-side maintenance call; `TestOnceReportsContextCostAndTurn1Usage` covers the Turn-1/Turn-2 half end to end.
NOTES (2026-09-17): the `context cost:` line prints full counts (not `format.Tokens`' coarse "1k") so the measured/estimate difference stays visible; `Turn1Usage.Calls` is 1 once a reading latched, the latch key.
...
## 6. `apogee probe context --live` — the measured columns — ✅ DONE (2026-09-17)

NOTES (2026-09-17): `TestProbeContextLiveSendsTwiceWhenReactionsArmed` asserts two requests, both columns and the delta line but NOT "the first carrying the advise directive": a user-origin `advise:` fires only at post-tool-result or file-changed (domain.Reaction.Validate, ADR 0076 D2), neither of which precedes Turn 1, so no config-armable Reaction can put a directive on the Turn-1 wire — the sync lane is armed as run.Once arms it, but the wire cannot show it; for a config-armed Reaction the delta line reads 0 (a delta above 0 needs an engine/bench-origin shape Reaction). The test comment states this.
NOTES (2026-09-17): the calibrated ratio in the header is derived from the calibrated report (total bytes over total tokens) — the estimator's ratio is not on the public Agent surface and the item's Files list stays out of internal/agent; `%.1f` hides the ceiling's rounding.
NOTES (2026-09-17): the reply ceiling is pinned through `Config.Context.MaxOutputTokens` (the loop's own cap seam, `newProjection`) rather than a pre-request hook; the stub sees `max_tokens: 8`.
NOTES (2026-09-17): a startup entry that names no model is resolved as `probe model` resolves its label (the server's `ActiveModel` via `provider.NewClient(...).Discover`), then the Config is recomposed on it reusing the already-resolved key (`probeContextConfig` gained a `probeContextOverlay` parameter); no model from either source refuses (`errProbeContextLiveNeedsModel`).
NOTES (2026-09-17): the estimate rows on a two-column report are the first (as configured) Agent's calibrated report; the second Agent's report is the same bytes.
NOTES (2026-09-17): two tests beyond the item's list — `TestProbeContextLiveCarriesTheCachedShare` and `TestProbeContextLiveRefusesAServerThatReportsNoUsage` (a reply with no usage is a refusal, never an estimate dressed as a measurement); the live report drops the estimate's armed line since the delta is the measurement it pointed at.
NOTES (2026-09-17): consequential edit — cmd/apogee/doc.go: made necessary by the `--live` flag (the probecontext.go map line now names it).
NOTES (2026-09-17): consequential edit — internal/probe/doc.go: made necessary by the measured columns (the contextcost.go map paragraph no longer says the Agent always sends nothing).
NOTES (2026-09-17): the `--endpoint` flag help now reads "(nothing is sent without --live)"; the manual's `--live` paragraph is item 7's.
