...
- **Authoritative sources:**
  - `docs/layout/settings-screen-layout.md` — the owner's layout mockup + 7 requirements. When an item's text and this spec disagree, the spec wins.
...
**What:** The layout spec's visual requirements (`docs/layout/settings-screen-layout.md` lines 1–10 and the mockup), in `internal/tui/settings.go` (+ `popup.go` only if the pane title needs it):
...
NOTES (2026-08-06, correction): (e) still holds for the value sub-list and the armed reset, but this item's mouse layer turned out to be incomplete against spec requirement 7 in two ways, both repaired later in `70f2de2`. The multi-line `system-prompt-text` field item 14 went on to add took no pointer at all; it now answers click, drag and wheel over every one of its rows through its own geometry (`settingsTextPaint` / `handleSettingsTextClick`, with `popupPlacement` carrying the per-row blocks and `popupWrapOffsets` reading a wrap backwards). And a bracketed paste — the one edit that arrives as neither key nor click — was routed by `Model.Update` on `inputEditable` alone, so it landed in the chat box drawn *under* the full-height pane; it now goes through `Model.settingsOwnsInput`, the same rule `handleKey` routes keys by.
...
NOTES (2026-08-06, correction): two of the claims above were reversed by a later fix, `70f2de2`. (e)'s "the mouse takes no pointer in this state at all" is no longer true — the field answers all three gestures over all of its rows: a click seats the caret at the glyph under the pointer, on a wrapped continuation as readily as on a line's own row; a drag selects across the lines and its release copies exactly those runes; the wheel walks the prose a line at a time. `renderSettingsText` split into a `settingsTextSpec` so painter and pointer compose the field once. And `lineEditor.noPaste` is DELETED — the widget's own ctrl+v is back on in BOTH settings fields, because the clipboard reply is a Msg of the widget package's own unexported type and is therefore claimed on `Update`'s default arm by the surface that owns the keyboard rather than by its type. The single-line field records its confinement (`oneLine`) and folds a pasted line break onto its row instead of refusing the paste.
...
## 16. The $EDITOR jump: suspend, reload, diff, apply — ✅ DONE (2026-08-06)

Depends on items 4, 8 and 12.

NOTES (2026-08-06): six deviations from the item's literal text. (a) `ReloadConfig` re-reads, validates and DIFFS; it does not apply. The pane applies each returned key through its own `settingsApplyLive` — the very function an in-pane commit uses — because the five renderer-owned display keys (`auto-title`, `ui.spinner`, `ui.spinner-color`, `ui.show-scrollbar`, `cursor-shape`) have no dispatcher entry BY CONSTRUCTION (they are Model fields, ADR 0037's other apply home), so a root-side apply loop would answer "cannot be applied to the running session" for five keys that apply perfectly well, and a root that skipped them would need a second copy of internal/tui's list. One apply path for both halves of the surface; `tui.AppliedSetting` therefore carries `{Path, Value}` and the note and the error are produced per key by the pane, exactly as they are for an in-pane commit. (b) `server` is never reported by the reload. Its live move is a deliberate act at the picker (ADR 0037 decision 4) and the recorded key IS the startup binding (ADR 0036 decision 2) — a file re-read does not move a session, and journaling it would put a ` *` beside a switch that never happened. It is now the permanent no-live-seam probe in `TestRunRootWiresTheLiveApplySeam`, which used `servers` until this item gave that key a home. (c) `SettingRow.ExternalEdit` is a new DECLARED affordance rather than a shape the renderer reads off the row — item 12's `kindServer` precedent: `unconfined-hosts` is structured and read-only too and ADR 0012 fences it out of the editor, so "which read-only rows open an editor" is a fact about the schema, and the flag and the pointer wording both come off one predicate (`externallyEdited`) so they cannot name different sets. (d) The diff's baseline is the FILE's own projection — `applyConfig` re-run with no flags and no environment, taken at spec time, projected through `settingsRows` — not the session's resolution: comparing against the resolution would report every env-overridden key as changed on the first round trip, and comparing against a startup snapshot would re-report every in-pane edit made since. That also gives the item's "parse/validation failure applies nothing" for free, and an undetermined startup server is deliberately NOT such a failure (it is the pre-bound state ADR 0036 answers with a picker, and the state a config is in halfway through gaining its first entry). (e) The holder update the item sites inside `ReloadConfig` is instead three new dispatcher entries — `servers`, `mechanisms`, `validated-sets` — each re-reading the file the way `reloadSystemPrompt` already does for the prompt trio, which is what (a) requires and what keeps every structured key's apply in the one place a key's apply lives. (f) The `+line` jump falls back to the key's COMMENTED EXAMPLE line when the file does not set the key (the seeded template documents every key, and that is where a human goes to set one), and a document the splice parse refuses opens with NO jump rather than refusing to open: an unparseable config is the file most in need of an editor. Also worth knowing: the pane's single failure slot means a reload in which two keys both refused shows the last of them (the slot describes an attempt, not a row — `settingFailure`); an editor exiting non-zero ends the trip without a re-read, because that is how an editor says discard (`:cq`); and binding C's mid-run refusal is reached in practice through `actuation.inFlight` alone — a streaming Step leaves this pane's keys unrouted entirely, the overlay being idle-only.

NOTES (2026-08-06, correction): (d)'s baseline is still the FILE's own projection — `applyConfig` re-run with no flags and no environment — but "projected through `settingsRows`" alone was not enough, and a later fix, `b5e43fd`, widened it. The baseline is now a `fileProjection` carrying BOTH the rows and the resolved `options` behind them, because six keys paint only a SUMMARY of what they hold ("1 server", "on, 2 aliases", "harmony, thinking delimited") and a summary is the right thing to paint and the wrong thing to diff: repointing the one `mcp-servers:` entry at another machine leaves the row character-for-character identical, so the change was reported by nothing, applied by nothing, and sat in the file waiting for a relaunch — the deferral ADR 0037 exists to abolish. The rows still say what a changed key carries back (`appliedValue`); the resolved values now say WHETHER it changed (`settingChanged` over a `settingStructures` table pinned to the registry by `TestSettingStructuresCoverEveryStructuredKey`). Those values are compared, never rendered — a `servers:` entry carries an api-key.
...
## 17. MCP reconnect and model-profile through the reload path — ✅ DONE (2026-08-06)

Depends on items 5, 15 and 16.

NOTES (2026-08-06): five deviations from the item's literal text. (a) `internal/mcp` needed NOTHING: the teardown the item asks for is already there and already tested — `Client.Close` (`client.go:149`) tears down every session, is idempotent, and `TestClose_TearsDownSessions` plus the connect-after-close test cover it. What DID have to move is the run's own teardown: the root's `defer mcpClient.Close()` closed the client the process connected, which after a reconnect is the set that was already closed hours ago while the live one leaked — it is now `defer mcpSet.close()`, which closes whatever the holder ended on. (b) The reconnect is exercised against a narrow `mcpSession` seam (`Tools`/`Close`, `*mcp.Client`'s own shape) with a fake in the tests rather than an in-process fake MCP server: what this item added to the composition root is the ORDER of the act — dial, swap, tear down, and each failure's way back — and the dialling itself is `internal/mcp`'s, whose own suite already connects to a real fixture server over a real stdio transport. (c) `liveTools` now REMEMBERS the web-search endpoint it was built from (a third constructor argument): a reconnect rebuilds the whole set, and rebuilding from the startup snapshot would silently revert a `web-search-endpoint` edit made an hour earlier — one key nobody touched changed by a key somebody did. (d) `lateEngine.SetProfile` is REMEMBERED while unbound and installed at the bind, unlike `SwapTools` which refuses there: a registry has a carrier across the bind already (`Config.Tools`, which the root holds), while the profile a bind would otherwise install is the one the process started with — so a pre-bound edit that was remembered nowhere would be an edit waiting for the next launch, which is what ADR 0037 abolishes. Its validation stays the Agent's, so a dialect this build cannot parse now makes the BIND fail (the Agent is released and the holder left free) — exactly what a config carrying that profile at launch already does. (e) `TestApplySettingRefusesWhatItCannotApply`'s "a key with no live seam" case moved off `mcp-servers`, which now has one, onto `server` — the key that has none by design and permanently (item 16's NOTES named it for this).
...
## 18. Docs sweep: README, layout.md, CONTEXT.md — ✅ DONE (2026-08-06)

Depends on items 1–17.

NOTES (2026-08-06): four deviations from the item's literal text. (a) A FOURTH doc was reconciled — the layout spec this plan calls authoritative. Its path in the header above is stale (it moved to `docs/layout/settings-screen-layout.md` in commit 3459dee), and its mockup was stale in three ways against what items 9–14 built: it still drew `system-prompt-file none edit in config.yaml` for a key item 13 promoted, it spliced the pane title into the top border where the build spends a title ROW, and it led every key row with `·`, which in the build means one specific thing (the last cell — the note or the pointer). It now carries the owner's seven requirements VERBATIM — they are the owner's words and were not rewritten — followed by a dated "As shipped" section: renderer-captured mockups of the five states (key list, edited row, selection popup, single-line field, multi-line field) and a point-by-point list of where the sketch and the build differ. (b) `grep -n "next launch" README.md` finding nothing costs two phrase swaps OUTSIDE the settings section: L367 and L448 use those words for the honest thing — where the `server:` line a switch records makes the next start begin — and not for the abolished deferral marker, so they read "next start" now. The alternative was failing this item's own acceptance on two lines that were never the marker. For the same reason the settings section DESCRIBES the override note ("the override will win again the next time apogee starts") rather than quoting the row's `· … outranks at next launch` string. (c) The CHANGELOG's `[Unreleased]` `/settings` entry was rewritten, though this item does not name that file: it promised `→ <value> (next launch)` markers, `· edit in config.yaml` pointers and read-only structured blocks to a release that has not shipped yet, so leaving it would have shipped release notes for a surface nobody built. No release heading and no version identifier was touched. (d) Nothing was removed from `ISSUES.md`/`TODO.md`: neither holds a settings-editing deferral this plan resolves — `ISSUES.md`'s single entry is the pre-existing `/server` rebind-inputs split, which this plan did not touch, and `TODO.md`'s two `config.yaml` entries are the tool×mode matrix and the url-safety key, both untouched deferrals of their own.
