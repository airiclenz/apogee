...
## 3. Heartbeat and probes — the remaining three wires, and the report's presence line — ✅ DONE (2026-07-27)

NOTES (2026-07-27): the absent-key value reads `none — no api-key: in config.yaml and no APOGEE_API_KEY (a local server needs none)` rather than the plan's bare `none` — the endpoint line's own not-configured text already names its sources, and "none" alone reads as a fault on the keyless local server that is the norm. Placement: the line sits directly under `endpoint` in both the reached and the unreached shape (it is the endpoint's credential), and is deliberately ABSENT from the third, not-attempted shape, which the plan did not list — a key verdict about a server that was never contacted is noise; `TestHostReportOmitsAPIKeyWithoutAnEndpoint` pins that. Each of the three seams also got a companion empty-key test (no `Authorization` header at all, not an empty one), which is decision 3's byte-identical promise proven where the wire is, beyond the tests the item enumerates.
...
## 4. Template, docs, and release bookkeeping — ✅ DONE (2026-07-27)

NOTES (2026-07-27): three shapes beyond the item's literal text. (a) README: besides the key's entry in the precedence paragraph and the softened line-14 claim, the key got its own `### The upstream API key` subsection — the file documents every non-trivial setting that way (system prompt, present:, blast radius), and a paragraph tacked onto the precedence prose could not carry the file/env example, the no-flag reason and the plain-text caveat without swamping it. (b) `technical-design.md`: amended the **Provider / Upstream** row (the construction-inputs enumeration the item names) AND, in the CLI/probe row, the host report's field list, which had become stale the moment item 3 added a line to it — five words, in place. (c) `defaults_test.go` made no key-presence assertions to extend, so the template check is a new `TestEmbeddedDefaultConfigDocumentsTheAPIKey`; the existing behaviour-neutrality test already pins that the shipped block stays commented.
