...
## 8. ADR 0069 — the top-level model picks the delegation seat; ADR 0045/0066/0039 amendments — ✅ DONE (2026-09-01)

NOTES (2026-09-01): the regression guard's grep (`human-chosen|stays deferred|Model-chosen` over docs/ + CONTEXT.md) also hits four ARCHIVED plan documents (`docs/plans/archived/2026-08-11 - 07 - sub-agent-server-plan.md:69`, `2026-08-31 - 03 - sub-agents-server-root-key-and-picker-plan.md:26`, and two unrelated `stays deferred` hits about headless/skills). Archived plans are executed historical records — item 1 of this run set the same precedent for its own archived-plan hit — so they are left as written; the two live claims (ADR 0045's Deferred bullet, ADR 0066 decision 7) are both amended in place.
NOTES (2026-09-01): ADR 0066 decision 7's and ADR 0039 decision 3's original text is preserved verbatim with a `> **Amended <date> by [ADR 0069](…)**` blockquote beneath, ADR 0025's / ADR 0022's house idiom (the same form item 1 used on ADR 0022's addendum); the two narrower scopings — ADR 0066's "Concretely" lead-in and decision 1's "every delegation at every depth routes there" — are inline `*…*` parentheticals instead, because they scope a clause rather than supersede a decision.
NOTES (2026-09-01): consequential edit — docs/adr/0066-…:138 Consequences bullet: made necessary by ADR 0069 — its "its deferred items stay deferred" claim about ADR 0045 is now false for the Model-chosen-routing half, so it gains a dated note naming which half is ratified and which still is not.
NOTES (2026-09-01): consequential edit — CONTEXT.md **Sub-agent** entry (:136): made necessary by this item's Parallel-agents change — the "concurrently up to the server's Parallel agents cap" sentence gains "a reply split across Delegation seats takes the smaller of the two caps", so the cross-reference does not read as false for a mixed reply. Prose only.
NOTES (2026-09-01): CONTEXT.md's **Orientation block** entry also gains the ADR 0023 §6 half of the item's regression guard — the Delegations line "carries no availability state and moves only on the human doors (`/server`, `/model`, `/sub-agents-server`), the way the Scratch dir moves at a session boundary" — beside its existing per-session-constant sentence, so the KV-cache claim stays true with the new line in the block.
...
## 12. Engine: mixed-seat width and the orientation Delegations line — ✅ DONE (2026-09-02)

NOTES (2026-09-02): the Delegations bullet is inserted AHEAD of the context-files bullet (orientation asset index 4, context files moving to 5) rather than appended after it as "asset gains line 5" reads literally. The context-files bullet is the block's bridge to the fenced blocks that follow it — orientation.go's own doc says "the last bullet is the one that speaks about what follows the block" — and a harness bullet between the bridge and what it bridges to reads as a break in the prose. The constants are `iota`-based, so nothing but the asset moved.
NOTES (2026-09-02): the asset line carries ONE `%s` for the rendered clause list rather than the item's three (`run_on "session" = %s; run_on "sub-agents-server" = %s; unset = %s`), and the fixed `run_on "<value>" = ` text is composed in orientation.go from `tools.RunOnSession` / `tools.RunOnSubAgentsServer`. Two reasons: a fixed three-`%s` template cannot DROP the sub-agents clause, which the item's own Tests line requires when no seat is installed; and spelling the enum's two values as prose in the asset is a second copy that can drift from the schema the model actually reads. The RENDERED line is byte-identical to the item's text in the both-seats case, and the asset keeps the file's one-`%s`-per-bullet convention.
NOTES (2026-09-02): internal/agent/agent.go is added to Files — the seat holder is a `seatMu`/`seat` pair on the Agent beside the `delegation` latch ("held beside the latch"), and a struct field can only be declared where the struct is. delegationseat.go holds the type, the `SetDelegationSeat` door and the `subAgentsSeat()` read seam, as the item specifies. A plain mutex+pointer rather than a holder type of its own keeps construct.go out of the change: the zero value is the honest "no seat installed", and the seat is never shared with children (the offer is depth-0 only).
NOTES (2026-09-02): consequential edit — internal/agent/dispatch.go `fanOutWidth`'s doc said "dispatchTools calls it ONCE per reply"; dispatchTools now calls `fanOutWidthFor`, so the sentence names the new caller. The function body, signature and behaviour are untouched, as the guard requires (`TestFanOutWidth_BoundsTheGroup` unchanged and passing).
NOTES (2026-09-02): `delegationWidth`'s doc gains the item's "documented as the batch hint" half — that `LoopView.ParallelAgents` (loop.go:877, :1408) stays the DEFAULT seat's width because a Mechanism-synthesised delegation names no `run_on`. Documented there rather than at the two `req.SetParallelAgents` call sites so the rule lives in one place; loop.go is untouched.
NOTES (2026-09-02): consequential edit — internal/agent/doc.go's file-count sentence read "Twenty-one files" while the package already held 23 non-test files before this item; adding delegationseat.go makes 24, so the count is corrected with the map line it belongs to. The staleness predates this item; leaving it while adding a file to the very sentence it counts would have made it further wrong.
NOTES (2026-09-02): consequential edits, comment text only, all inside listed files — orientation.go's file doc (it enumerated the facts the block states), its positional-asset comment ("the three PATH bullets ... exactly one %s") and `mustOrientationTemplate`'s doc ("the header plus the three bullet templates", already stale at four) now describe the bullet set as it is.
NOTES (2026-09-02): the full-block regression pin (`TestOrientation_PlainToolStatesNoDelegationsBullet`) composes its expectation from the asset templates rather than spelling the prose out, following orientation_test.go's stated convention that the tests pin the block's SHAPE so wording may be tightened. It still pins the whole rendered block byte-for-byte — no bullet added, dropped or reordered with the plain tool.
...
## 13. Host wiring: gate → roster, seat facts → engine, picker description cell — ✅ DONE (2026-09-02)

NOTES (2026-09-02): `registryWithMCP` takes the gate as its own `seatChoice bool` parameter, filled from `spec.seatChoice` at both call sites, rather than taking the whole `toolSetSpec`. The guard's requirement is that the gate travel with the tool SET and not through `apogee.Config`, which this satisfies; passing the spec as well would have given the function two sources of truth for the four fields the rebuild closure already folds into its `cfg` copy.
NOTES (2026-09-02): consequential edit — apogee.go and example_test.go: made necessary by `delegationSetter` gaining `SetDelegationSeat(*apogee.DelegationSeat)`. cmd/apogee reaches the engine only through the root package (it imports internal/agent nowhere), so the type needs the alias beside `DelegationTarget`'s, and example_test.go's alias census is the structural test that pins it.
NOTES (2026-09-02): consequential edit — cmd/apogee/wire.go `seatChoiceNote`: made necessary by the row moving from the holder class to the swap door. "applies at the next roster build" now tells the human nothing has happened yet, which this item makes false — the commit builds the set there and then — so the note becomes `tools.disabled`'s own boundary, "applies to the next request". The constant is compared by NAME in wire_settings_test.go, so no test spells the old string.
NOTES (2026-09-02): consequential edit — cmd/apogee/wire_tools_test.go: the eight `registryWithMCP` call sites gain the new argument (mechanical), and the file is the natural home for the item's "startup with `sub-agents-choice` absent builds the same roster" pin (`TestRegistryWithMCPCarriesTheSeatChoiceGate`: the gate moves a schema, never the roster).
NOTES (2026-09-02): consequential edit — cmd/apogee/wire_settings_test.go `TestLiveSettingsOptionsFollowEveryApply`: that census enumerates "the keys whose apply moves something OUTSIDE the holder", a class `sub-agents-choice` joins with this item, so it gains an eleventh row and its two counts are corrected. Its own behaviour test is renamed `TestApplySettingSubAgentsChoiceSwapsTheSeatGate` (from `…RecordsTheSeatGate`) because the key no longer only records, and a sibling refusal test pins that a refused swap leaves BOTH the set and the holder's word where they were.
NOTES (2026-09-02): consequential edit — cmd/apogee/upstream_test.go `TestServerChoicesCarryNoSecrets`: that test pins exactly which entry fields cross to the renderer, so the new `Description` had to be added to its expectation rather than left to fail as an unpinned field. `serverChoices` fills it for BOTH pickers (one projector, no duplicated knowledge) and only `subAgentsServerRows` renders it — pinned from both sides by the two new picker_test cases.
NOTES (2026-09-02): the "one pushed after bind reaches the Agent" half of the wire_engine_test case is asserted as far as this package can see it — `pendingSeat` follows every push and a bound push is accepted by a real Agent. `Agent.subAgentsSeat` is unexported, so the installed-on-the-Agent claim is pinned in internal/agent (item 12) and end to end in item 14, not here.
NOTES (2026-09-02): the seat is pushed on `relist`'s COMMIT path only. Both of its early returns are the cases where the installed entry did not move (the same entry re-read; a still-missing name whose notice is unchanged), so a push there would restate facts that did not change — and the ADR 0023 §6 rule this line lives under is that it moves on the human doors and nowhere else.
