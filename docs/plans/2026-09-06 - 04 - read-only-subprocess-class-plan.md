...
## 2. The `readOnlySubprocess` marker, carried by the git read trio — ✅ DONE (2026-09-07)

NOTES (2026-09-07): consequential edit — internal/tools/doc.go: the package file map (docmap.Check / TestDocMapNamesEveryFile) enumerates every non-test file, so the new readonly_subprocess.go needed an entry; made necessary by adding that file.
NOTES (2026-09-07): the git.go package header's preceding sentence ("the disposition runs all of them under Confiner.Confine in Auto and gates them...") is made false by the same change, so it was narrowed to the write pair (git_branch, git_commit) and the paragraph re-wrapped — the item's prose-rewrite rule is a floor, not a ceiling.
NOTES (2026-09-07): all three new/extended tests were confirmed to bite by reverting the change against them — dropping GitLog's marker method fails TestGit_Markers and TestReadOnlySubprocessNamesTheGitReadTrio; dropping gitDiffHardeningArgs from the log argv fails TestGitReadTrio_ArgvCarriesTheHardening/git_log.
NOTES (2026-09-07): test (c) landed as one table-driven TestGitReadTrio_ArgvCarriesTheHardening with a shared recordGitArgv helper in the TestGitCommit_PassesNoGpgSign shape; its git_status row re-asserts item 1's exact list as the plan's Tests line says, alongside the standalone TestGitStatus_PassesIgnoreSubmodulesDirty.
...
## 3. `classReadOnlySubprocess` takes the RO row in every mode — ✅ DONE (2026-09-07)

NOTES (2026-09-07): consequential edit — internal/agent/resolution.go: `gateReason`'s doc comment counted "Six of the seven classes"; the eighth class makes that false, so it now reads "Seven of the eight". The gate reason/cacheKey logic itself is untouched, as the item requires — the trio never reaches it.
NOTES (2026-09-07): `dispatch.go` was read and needs no change, as the item anticipated: it switches on `verdict.kind` only, and `executeRun` passes a nil confinement box (its one `confineChildren` branch is keyed to `classWorkspaceWrite` in Auto), so a `resolveRun` verdict for the trio runs `Execute` with no box.
NOTES (2026-09-07): `TestResolve_LadderTable`'s case struct was anonymous, which cannot be appended to; it is now a named local type `ladderCase` (fields and every existing row byte-identical) so the RO-subproc block can append its generated cross-product — 60 rows: 3 tools × 5 modes (Plan, Ask-Before, Allow-Edits, Auto, an off-ladder mode) × confine true/false × fs-confine true/false, all asserting `resolveRun`.
NOTES (2026-09-07): the unmarked `roSub` fake in `resolution_test.go` was named `"git_diff_range"` at both of its sites; since the real `git_diff_range` now takes the RO row, that name made the fake's refuse/gate/confine rows read as an assertion about the shipped tool. Renamed to `"ro_declared_subproc"` (the name `planmenu_test.go` already uses for the same shape); its rows and verdicts are unchanged.
NOTES (2026-09-07): `TestPlanToolMenuDropsTheDriftedPair` renamed to `TestPlanToolMenuDropsDiagnosticsAndTheUnmarkedFakes` per the item's "becomes" wording — the old name named a pair that is now a single tool. It still matches the acceptance command's `TestPlanToolMenu` prefix.
NOTES (2026-09-07): the new tests were confirmed to bite by deleting the `tools.IsReadOnlySubprocess` branch from `classifyTool` — 60 subtests fail (the ladder cross-product, the three `TestClassifyTool` rows and `TestPlanToolMenuOffersTheGitReadTrio`); `TestPlanToolMenuAgreesWithTheLadder` passes unchanged in both directions, as the item requires.
...
## 4. Contract, ADR 0012, CONTEXT.md and manual record the new class — ✅ DONE (2026-09-07)

NOTES (2026-09-07): (d) — the manual carries no sentence claiming the git read tools prompt, gate or are unavailable in Plan (`grep -rn 'git_status\|git_log\|git_diff_range\|git tools' docs/manual/` returns only a tool-name list in `configuration.md:133` and a diff-rendering sentence in `commands.md:191`, both still true), so the correction rule had nothing to correct; no `docs/manual/tools.md` exists. Per the item's regression guard the per-mode addition landed as one paragraph under `## Auto mode's blast radius` instead of a sentence per mode row.
NOTES (2026-09-07): consequential edit — docs/design/confinement-execution-contract.md: the 2026-07-26 amendment block and footnote ² each gained a dated scoping tail, made necessary by the new class (both said in the present tense that `git_diff_range` rides the subproc row and that Plan neither offers nor runs it).
NOTES (2026-09-07): consequential edit — CONTEXT.md: the **Ask-Before** and **Allow-Edits** bullets of the Agent-mode entry gained a clause excepting the git reads, made necessary by the Plan bullet's change ("every write, command, and external reach requires an Approval" and "shell/exec … still gate" were false for the trio).
