...
## 2. `title.FirstLine` and `title.DelegateLabel` replace the delegate-naming copies
...
## 4. The TUI reads the grammar from `internal/refs` and drops its copy
...
## 5. A Firing's prompt resolves its `@file` references
...
## 6. `probe.AutoUnattendedBlocked` is the one unattended-Auto verdict
...
## 7. `daemon.Host` carries the confinement facts and asks probe itself
...
## 8. `mechanisms.ResolveEnabled` validates the `mechanisms:` block and words the retired-id notice
