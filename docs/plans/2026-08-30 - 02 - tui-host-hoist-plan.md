...
## 9. `cmd/apogee` resolves Mechanism ids through the library — ✅ DONE (2026-08-31)

NOTES (2026-08-31): the plan's "converting `[]domain.MechanismID` to `[]apogee.MechanismID`" was a no-op — `apogee.MechanismID` is a type ALIAS of `domain.MechanismID` (`apogee.go:412`), so the slice types are identical and every call site assigns straight through.
NOTES (2026-08-31): deleted `fakeKnown` (`wire_tools_test.go:215-219`) alongside the plan's `:221-287` range — it was the stand-in catalogue for exactly those four deleted tests and would otherwise be dead code; `internal/mechanisms/retired_test.go` carries its own equivalents.
NOTES (2026-08-31): consequential edit — `cmd/apogee/wire_tools.go`'s file-top concern map dropped the same `mechanisms:`-validation clause the plan strikes from `doc.go`, since the functions it named left the file.
...
## 10. Headless and daemon runs say when a retired Mechanism is ignored — ✅ DONE (2026-08-31)

NOTES (2026-08-31): the expected line is built once by a shared test helper, `retiredMechanismNotice`, added to `cmd/apogee/wire_helpers_test.go` — the package's shared-test-helper home — because all three Driver tests need the same string; the item's Files list did not name that file, so it joins FILES as a deliberate deviation. The release comes from `mechanisms.RetiredRelease`, never retyped.
NOTES (2026-08-31): the item names `cmd/apogee/headless.go:341` and `daemonfire.go:93` as carrying the `// item 10 prints these` marker; both in fact carried it as prose ("item 10 of the tui-host-hoist plan prints/logs them"), as the run's DECISION recorded. Both are gone — `grep -rn 'item 10' cmd internal docs/manual ISSUES.md` returns only unrelated pre-existing hits (other plans' item 10s).
NOTES (2026-08-31): removing the `ISSUES.md` entry left a double blank line before the next `###` heading; it was collapsed to the one blank line every other sibling section in `## Open defects` uses.
NOTES (2026-08-31): each of the three new tests was falsified — with its Driver's print/log loop replaced by `_ = retiredNotices` the test fails with 0 occurrences — before the loop was restored.
...
## 11. `ISSUES.md` records the Driver-parity gaps; the review report joins the repo — ✅ DONE (2026-08-31)

NOTES (2026-08-31): the plan (this item's **Files**/**Acceptance** and the header's "Authoritative sources") names the review report `docs/reviews/architecture-review-20260830.html`; the file on disk is `docs/reviews/architecture-review-2026-08-30.html` (hyphenated date, the same form as `docs/reviews/code-audit-2026-08-25.md`). Per the run's DECISION the real filename is used everywhere — the ISSUES.md pointer, the acceptance greps and this item's FILES list — and the file is NOT renamed.
NOTES (2026-08-31): the item's "stage the review report" half was already satisfied when this item started — `docs/reviews/architecture-review-2026-08-30.html` was committed earlier in this run ("docs(reviews): save the 2026-08-30 architecture review report") and `git ls-files docs/reviews/` lists it, so it is absent from FILES and was neither re-added nor renamed. This item's commit therefore carries `ISSUES.md` alone.
NOTES (2026-08-31): two cited locators moved between the plan's base commit `a9ec2131` and this item's HEAD `c32c2825` — item 10 added ten lines to `cmd/apogee/daemonfire.go`, so `cfg, _, err := firingConfig(` is now `:217` (plan: `:207`) and the `schedule.Outcome` that carries no `res.Usage` is now `:247` (plan: `:237`). The entry cites the HEAD line numbers, since `ISSUES.md` evidence must resolve in the tree it ships with. Every other cited site (`internal/tui/model.go:742`, `cmd/apogee/wire_boot.go:278`/`:315`, `internal/tui/heartbeat.go:620`, `cmd/apogee/upstream.go:559`, `internal/tui/undo.go:45`, `cmd/apogee/root.go:105`/`:117`) was re-verified at HEAD and resolves unchanged.
NOTES (2026-08-31): each gap line gained a second, corroborating locator naming the Driver-side fact (`internal/run/run.go:67-111` and `:96-107`, `cmd/apogee/headless.go:371` and `:190-200`, `cmd/apogee/wire.go:120`, `cmd/apogee/wire_verbs.go:58`, `internal/agent/agent.go:742,761`) — the item asks for "its TUI site and the Driver that lacks it", and the register's own Conventions require actionable `file:line` evidence for both halves. All were verified at HEAD.
NOTES (2026-08-31): the section states in its Status paragraph that none of the nine is a regression (all pre-date this run, absent since each Driver was built) — `ISSUES.md`'s Conventions forbid a regression entering the register, so the claim is made explicitly rather than left implicit.
