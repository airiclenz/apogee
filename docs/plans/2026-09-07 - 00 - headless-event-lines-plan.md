...
## 7. Goldens: the contract under a scripted upstream — ✅ DONE (2026-09-07)

NOTES (2026-09-07): `not-started-after-sink.jsonl` is TWO lines — `run_started` at seq 1, `run_finished` at seq 2 — not the one line the item's guard sentence names. The opening frame is written unconditionally before `runOnce` (headless.go), and the `res.Turns == 0` refusal this golden records is the exit that follows it, so a one-line stream is unreachable on that path; the item-4 test `TestHeadlessFormatJSONFramesEveryExit` ("a run.Once pre-run exit refuses after the sink exists") already pins the two-frame shape. The subtest asserts exactly that and the one-line assertion is kept for `not-started.jsonl`, where the offline gate does refuse before any frame.
NOTES (2026-09-07): `GoldenText(t, path, text, redactions...)` keeps the arity the item's What names; the second argument is the golden's PATH relative to the package directory, which is how it "takes the golden's dir and extension" — it splits them and threads both into `compareGolden` (now `compareGolden(t, dir, name, ext, text, update, redact)`; `Golden` passes the new `frameExt` constant). Four adjacent string parameters would have been the alternative and is the shape the standards' clarity rule argues against.
NOTES (2026-09-07): the `not-started` golden's error redaction is `"error":"(?:[^"\\]|\\.)*"`, not `"error":"[^"]*"`: `notice.ServerOffline` quotes the URL it dialled, so the naive pattern stops at the first escaped quote and leaves the OS-specific half of the sentence (`dial tcp …: connect: connection refused`) in the golden.
NOTES (2026-09-07): the after-sink case dictates an answering beat (`swapAnsweringBeat`) and a canned runner; its home names an endpoint nothing listens on, so without the swap the offline gate — not the runner — would be the refusal recorded.
NOTES (2026-09-07): golden stability was checked by recording once and re-running the three cases four more times, each under a fresh `t.TempDir()`; `context_files.standing_tokens`/`system_share` do not move with the temp path's own spelling.
NOTES (2026-09-07): consequential edit — internal/tuitest/doc.go: made necessary by GoldenText (the package file map described golden.go as frames under testdata/frames/ alone).
NOTES (2026-09-07): consequential edit — docs/design/test-drivers.md: made necessary by GoldenText (its "Goldens" section stated the rendering-surfaces-only rule with no mention of the ADR 0075 §14 supersession or of a golden outside testdata/frames/).
