...
## 11. demorig: compositor — zoom on targets and the cursor — ✅ DONE (2026-09-25)

NOTES (2026-09-25): re-derived from the assumption that ZoomV2 accepts an absent factor — item 6's validator refused a factor of 0, which left the Goal's auto-fit unreachable; `ZoomV2.validate` in cmd/demorig/storyboard_v2.go now accepts 0 (absent = auto-fit, error text unchanged) and the ZoomV2 doc comment says so; covered by TestComposeAutoFitFactor.
NOTES (2026-09-25): interpretations of the Goal — the auto-fit margin is 2 cells on each side of the box; the zoom spans the whole section (v2 has no zoom hold), ramps default to 400 ms when zero and shrink in proportion when in+out outruns the section; a zoom target with no matching click target event resolves on the latest snapshot of the beat it matches; the cursor fade-in runs over the 300 ms before the first glide starts (it is fully shown when the glide begins), the fade-out runs 300 ms starting 1.2 s after the last click; the outline colour is Mocha crust #11111b and the ring stroke 3 design px; design px are pixels at scale 2, multiplied by scale/2 and the downscale to frame.width.
NOTES (2026-09-25): API for item 12 — `NewCompositor(board, take, schedule, CellLayout)` and `Compose(outputTime, rasterizedSnapshot) *image.RGBA`; the caller builds CellLayout from the Rasterizer (Size, geometry.Padding, CellWidth, LineHeight, frame.scale) and picks the snapshot for each ScheduledFrame.Source.
