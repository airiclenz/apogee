...
## 19. Strip the transcript-mutating arms — ✅ DONE (2026-09-16)

NOTES (2026-09-16): acceptance grep `grep -c 'm.layout()\|m.refreshViewport()'` over the eleven files: before 72 (commandrun 13, interject 7, model 20, autotitle 7, schedule 6, actuation 7, heartbeat 3, sessionsave 2, skillscmd 3, effort 2, keymigration 2), after 27 (commandrun 3, interject 4, model 11, autotitle 0, schedule 5, actuation 2, heartbeat 0, sessionsave 0, skillscmd 0, effort 1, keymigration 1) — 45 stripped.
NOTES (2026-09-16): the 27 that stay are outside this item's rule, not exceptions to it — no generation-bumping transcript method precedes them in their arm: the editor arms (submit's siblings in handleKey, stageInterjection, popDeferredCommand, the withdraw/restore arm, queueCommand, runDeferredCommands, `/compact`'s box reflow, finishWorker's restoreAskDraft, resumeRunning — item 21), the pane opens/closes and footer-only arms (openSuggestMenu, shift+tab, the schedule pickers, startProfileLoad/startServerActuation, acceptKeyMigration's re-opened picker — item 20), and the WindowSizeMsg arm plus layout/freshenTranscriptClamp/settle/refreshViewportAnchored themselves (item 18's own sites). No stripped call was followed by a paint-state read (`m.lines`, `m.lineTargets`, `m.cursor`, `viewport.*`), so no "stays with a comment" case arose; `refreshViewportAnchored` untouched.
NOTES (2026-09-16): three strips read the bump through a helper rather than a literal `m.transcript.*` call in the arm: `applyPendingRebind` (heartbeat.go — the bump is `applyRebind`'s note/refreshStartup, whose `noted` flag existed only to drive the refresh, now `_`), `startNewSession`'s two `resetSessionView` sites (commandrun.go — reset/addStartup/setRoot), and `foldCompactDone` (commandrun.go — the switch bumps on every path; finishWorker's own `layout()` between it and the stripped refresh already made the refresh redundant). `foldActuationDone`'s non-load/non-unload path (a `/stop-server` whose result carried no heading and no steps) had no bump before its refresh, so that refresh was a no-op repaint — stripped with its siblings; settle's key compare makes it a no-op still.
NOTES (2026-09-16): comment sweep folded in: the eventMsg arm's "layout(), not a bare repaint" rationale and foldInterjected's are restated as settle's height half (freshenTranscriptClamp) carrying the pane/band heights; enterRunning's doc no longer lists "the layout" among what stays the caller's; actuation.go's two "repaints on its way out" comments (foldActuationDone's move branch, recordLoadedProfile's doc) now name the repaint tail. No test needed rerouting through `step`/`settle()` — `go test ./internal/tui` and every `TestE2E*` golden passed unchanged; `TestInputAutoGrowReflowsViewport` untouched.
