...
## 12. stubllm plays in-process: the engine's pure fakes collapse onto one Script — ✅ DONE (2026-09-16)

NOTES (2026-09-16): the listener-less constructor is `stubllm.InProcess` — the plan's example name `Load` is already `Load(path)` (the on-disk fixture reader).
NOTES (2026-09-16): the in-process transport lives in a new file `internal/stubllm/transport.go` (+ `transport_test.go`) rather than inside the 600-line `server.go`; `doc.go`'s file map gained its line.
NOTES (2026-09-16): `scriptResponder(t, script)` returns `*scriptedUpstream` — the real `*provider.Client` embedded (Stream/Close/SetModel are the client's own) beside the `*stubllm.Server`, so a test reaches the request log (`requests()`, `last()`, `calls()`); `scriptedResponder(t, turns...)` and `echoResponder(t, reply)` keep the retired fakes' names as constructors so ~330 call sites changed only their spelling.
NOTES (2026-09-16): survivors, each with its last user awaiting item 13's request log (sampling and effort): `recordingResponder` (agent_test.go's two EffortDialect/ThinkingEffort tests), `captureAllResponder` (rebind_test.go's Sampling.MaxTokens test), `capturingResponder` (budget_test.go's two Sampling tests), `cappedSummaryResponder` (embedded by `maxTokRecordingResponder`). Every other user of the eleven moved.
NOTES (2026-09-16): two id-less native-call tests (loop_test.go) stay on the existing `scriptedDeltas` fake — stubllm numbers an id-less call by position, so the shape under test cannot come off the wire; named here per the guard.
NOTES (2026-09-16): Delta builders shared with the ten surviving fakes (`contentScript`, `toolCallScript`, `subAgentCallScript`, `narratedToolCallScript`, `cappedChildTurns`, `retryableErrorScript`, `askUserCallScript`, `namedDelegationScript`) keep their Delta form and gained stubllm twins (`contentTurn`, `toolCallTurn`, `subAgentCallTurn`, `narratedToolCallTurn`, `narratedChildTurns`, `retryableErrorTurn`, `askUserCallTurn`, `namedDelegationTurn`); builders with no surviving user were converted in place (`errorScript` is now an `http: 500` turn carrying its message, `usageScript`/`usageToolCallScript` take `stubllm.Usage`, `emptyScript`, `cutOffScript`, `thinkingOnlyScript`, `cappedWithTextScript`, `draftWithToolCall`, `twoToolCallScript`, the list builders).
NOTES (2026-09-16): assertions recast to the wire, intent kept — fault texts are matched by `strings.Contains` on the scripted message (the provider wraps it in its own wording: `overflow_test.go` `lastFaultMsg`, `TestRespondAndReviewReStreamsATransientFaultOnce` scripts distinct "first blip"/"second blip"); `TestRetryExchange_CarriesSupersededAssistantAndCorrection` registers the `lookup` tool because the provider projects an assistant message's tool calls onto the wire only when the request offers native tools (`formatMessage`); `TestRetryExchange_EmptyInjectIsBareRestream` compares Messages+Tools (a log entry carries N and At); `usagetally_test.go`'s served-model test plays a Script whose Model is `servedModelID` (`servedResponder`).
NOTES (2026-09-16): acceptance grep `Stream(.*provider.Request) iter.Seq\[provider.Delta\]` over internal/agent/*_test.go: 28 at HEAD → 21 after (the four survivors above plus the ten out-of-scope fakes and their kin).
NOTES (2026-09-16): consequential edit — docs/design/test-drivers.md: made necessary by the new `InProcess`/`Handler`/`Transport` surface, the completion kind rule and the `chunks:` keys.
NOTES (2026-09-16): consequential edit — internal/stubllm/script_test.go: the pinned one-kind wording changed with the rule; the YAML round trip gained a chunked narrating turn.
