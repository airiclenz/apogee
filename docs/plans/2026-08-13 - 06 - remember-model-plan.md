...
## 6. /settings row for remember-model — ✅ DONE (2026-08-13)

NOTES (2026-08-13): the item's first two bullets were already in place (item 1's NOTES record why the registry row and the `/settings` value formatter had to land with the schema key), so this item is its third: the live apply. `internal/config/registry.go` and `cmd/apogee/settingsrows.go` are therefore untouched.
NOTES (2026-08-13): the live-apply case does NOT live in `cmd/apogee/settingsrows.go` where the item's What/Files place it — that file is the display table (sections, per-key value formatters) and holds no dispatcher. The `/settings` apply dispatcher is `applySettingFor` in `cmd/apogee/wire_settings.go`, so the `remember-model` case landed there, beside its line in the `unreachable` nil-guard the dispatcher's own drift test (`TestApplySettingRefusesEveryKeyItCannotReach`) pins against it.
NOTES (2026-08-13): the item's premise "the record seams and the boot check read current Options; no re-wiring needed" was false, exactly as the item-5 verifier found and the dispatch DECISION directs: `rootWiring.opts` is never reassigned after `newRootWiring`, so all three readers of `w.opts.RememberModel` were frozen at launch and a flipped toggle governed nothing until the next start. Fixed by giving `liveSettings` — the holder that exists for precisely this (ADR 0037 decision 1) — a `rememberModel` field with `remember()` / `setRememberModel()`, seeded from the launch snapshot, and routing all three readers through it: `recordModelChoice` and `recordLaunchProfile` (`cmd/apogee/wire_verbs.go`) and the `launcherWiring.remember` closure (`cmd/apogee/wire_live.go`). Those two files are outside the item's Files line; without them the case would store a value nothing reads.
NOTES (2026-08-13): the toggle is the one applied key that neither pushes at an engine seam nor rides a rebind — everything it gates is still in the future — so the holder store is the whole apply and the case returns no row note. Its `unreachable` line is `a.live != nil` alone.
NOTES (2026-08-13): tests landed in `cmd/apogee/wire_test.go` (dispatcher level: the flip reaches the holder, flips back, refuses a non-bool, drives no engine seam) and `cmd/apogee/upstream_test.go` (end to end through `runRoot`: a session started with the toggle off records the next `/model` pick and the next committed profile load after `ApplySetting("remember-model","true")`, and stops on the flip back), rather than beside `registry.go` / `settingsrows.go`. The item's other two test bullets are already covered where item 1 left them: `TestRegistryIsBijectionWithFileConfig` and `TestRegistryRowInvariants` pin the registry row, `TestSettingsRowsFormatEffectiveValues` pins the pane row's value, and the bool write vocabulary is enforced generically for every `KindBool` key by `renderSettingValue` and swept by the registry-wide writer tests.
