...
- `<tool-top-level-details>` — the right-aligned outcome slot. It carries the
  ...
  **Colour:** the `tool-marker` role while the block is collapsed and
  ...
  the marker tone, as does a run that `ended without a report` (it reached its
...
| sub_agent | Sub-agent | its name (task head fallback) | `scheduled` before it starts, else `N steps · done/failed/ended without a report` | task text + result summary |
