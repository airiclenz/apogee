...
| Confinement | S (iface)→design done (P3.1)→**P3.2 Linux landlock landed** | capability matrix; **ADR 0012**: blast-radius, Auto fs-only (`confine-to-workspace`-tunable, network open); the deferred design session is done — **[`confinement-execution-contract.md`](confinement-execution-contract.md)** pins the `Confine(*exec.Cmd)` prepare-in-place signature (closure form deleted), the `workspaceScopedWriter` marker, the per-call disposition table, capability honesty, and the shared escape-probe harness. **P3.2** ships the landlock backend (raw `x/sys`, ABI probe + honest caps + re-exec wrapper) + `confinetest` harness | P3.3 seatbelt (reuses `confinetest`) + dispatch wiring (P3.4: adopt the `*exec.Cmd` `Confiner` signature, wire `Confine` into dispatch, `__confined-exec` dispatcher in `main`) |
