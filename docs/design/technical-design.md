...
| Confinement | S (iface)→**design done (P3.1)** | capability matrix; **ADR 0012**: blast-radius, Auto fs-only (`confine-to-workspace`-tunable, network open); the deferred design session is done — **[`confinement-execution-contract.md`](confinement-execution-contract.md)** pins the `Confine(*exec.Cmd)` prepare-in-place signature (closure form deleted), the `workspaceScopedWriter` marker, the per-call disposition table, capability honesty, and the shared escape-probe harness | backend impls (P3.2 landlock / P3.3 seatbelt) + dispatch wiring (P3.4) — now mechanical against the contract |
