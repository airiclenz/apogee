...
| Platform (shell/path) | ∅→**P (P5.6/P5.7)** | one interface, POSIX v1, Windows Phase 5; **P5 item 6: the surface is `Shell{Command, CommandLine, Quote, ScopeEnv}` + `Path{ExecExt, Contains}`, and both rule sets live in ONE untagged table (`internal/platform/host.go`) compiled on every target — only `Current()`'s choice is build-tagged, so Windows semantics are table-tested from a Linux run. `CommandLine` returns the verbatim process command line for `SysProcAttr.CmdLine` ("" on POSIX): Windows has no argv at the syscall boundary and `os/exec`'s `EscapeArg` join produces quoting `cmd.exe` does not understand. `Contains` is the case-folded containment ADR 0020 needs — it normalises `\\?\`/`\\?\UNC\`, refuses drive-relative (`C:work`) and device (`\\.\…`) paths as non-locations, and treats an 8.3-shaped component as "resolve, else refuse" through an injected `longPath` seam (`GetLongPathNameW`, walking up to the longest existing prefix). **No `LookPath` wrapper** — `os/exec` already implements per-OS lookup including `%PATHEXT%`, so one would be dead surface; `ExecExt` is the one lookup-shaped fact it does not expose. Adopted callers: the `terminal` tool carries `spec.cmdline`, and `safeGitEnv` runs through `ScopeEnv` (POSIX output byte-identical — its floor is empty by design). **P5 item 7: process-*tree* teardown on Windows** — an unnamed Job Object created between `Start` and `Wait` with `KILL_ON_JOB_CLOSE`, terminated by `cmd.Cancel`; the decision half is the untagged `planTreeKill`, which the POSIX path now shares** | (none — both Phase-5 widening markers, and the teardown one, are retired) |
