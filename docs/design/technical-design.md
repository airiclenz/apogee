...
| Security guardrails | ∅→**P (P3.6)** | Approval, path/url safety, dangerous-action guard, circuit-breaker, audit; **P3.6: `internal/security` filled (D6 / ADR 0012) — human-in-the-loop layer, all modes, distinct from the `Confiner`. Path-safety **consolidated** here (`ResolveInRoot`/`EvalRealPath`/`ErrPathEscape` — verbatim move; the 4 built-ins delegate via thin aliases, parity preserved); `URLGuard` (scheme/host allow-deny, deny-first) for P3.11; the **dangerous-action guard** (two tiers — Tier-1 hard-refuse / Tier-2 force-approval — default-on, precision-over-recall, whitespace-normalized literal/regex, no obfuscation-chasing) with config-merge (`MergeDangerousRules`: global add OR remove, project add-only); a circuit-breaker (trips on N identical failing calls); an append-only audit log. `security.Guards` bundles dangerous-action + breaker + audit and is wired through `internal/agent/dispatch.go` (`PreExecute` runs **first**, tighten-only, ahead of the unchanged `needsApproval`) so all tools — and sub-agents (D2) — inherit them** | config.yaml surfacing of custom rules + breaker threshold (deferred); url-safety wiring (P3.11); the per-mode blast-radius disposition rework (P3.4) |
| Confinement | S (iface)→design done (P3.1)→**P3.2 Linux landlock + P3.3 macOS seatbelt landed** | capability matrix; **ADR 0012**: blast-radius, Auto fs-only (`confine-to-workspace`-tunable, network open); the deferred design session is done — **[`confinement-execution-contract.md`](confinement-execution-contract.md)** pins the `Confine(*exec.Cmd)` prepare-in-place signature (closure form deleted), the `workspaceScopedWriter` marker, the per-call disposition table, capability honesty, and the shared escape-probe harness. **P3.2** ships the landlock backend (raw `x/sys`, ABI probe + honest caps + re-exec wrapper) + `confinetest` harness; **P3.3** ships the seatbelt backend (`sandbox-exec` profile from box, presence-probe + honest caps, network-open-by-default) reusing `confinetest` (macOS live-probe owner/CI-deferred) | dispatch wiring (P3.4: adopt the `*exec.Cmd` `Confiner` signature, select the real backend per-OS — landlock/seatbelt/`denyConfiner`, wire `Confine` into dispatch, `__confined-exec` dispatcher in `main`) |
