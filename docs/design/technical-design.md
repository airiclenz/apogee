...
│   ├── tools/           # 21-tool suite + registry/executor
...
| Errors | `ErrAutoUnavailable`, `ErrConfinementUnavailable` (P3.2 — confine-if-you-can/gate-if-you-can't net), `ErrOrderingCycle`, `ErrSessionVersion`, `ErrInputPending`, `ErrNoOpenExchange` (2026-07-27 — `Interject` without an open Exchange; the caller wanted `Submit`) | 0003, 0012, 0025 |
...
| Loop / Turn engine | S→**P (P1.2)** | Turn = one primary Upstream call; quiescent boundary; recover-at-boundary (0007); **P1.2: the full Step — stream → parse → post-response hooks → tool dispatch through Approval → post-tool-result → boundary, emitting typed Events; `Run` steps until the Exchange ends; streaming+Approval interleave (§6 #6) and event delivery (§6 #3) settled; ActionDefer feed-forward survives snapshot; cancel mid-stream + mid-tool roll back; tool/hook panics recover — all under `-race`; engine adopts `domain.Conversation`; P1.6: the snapshot envelope now serializes the loop counters (`turnIndex`, in-Exchange flag, pending input) so Resume continues; Prompt-seam: `toProviderRequest` injects a non-native profile's `InstructionsFor` block as a wire-only system message (appended to a hook-seeded system message, else a sole one at position 0) and suppresses the native `tools` array — re-rendered per request over the mode-filtered menu, never persisted to history/snapshot; native/zero profile is byte-identical** | inline thinking-strip wiring (needs a `ThinkingConfig` source — Phase 2/3); sub-agent nesting (Phase 3) |
| Provider / Upstream | S→**P (P1.1)** | openai-compatible; model discovery; TS as oracle; **P1.1: real `internal/provider.Client` — non-streaming `Respond` + streaming `Stream` (`iter.Seq[Delta]`), bounded retries/timeouts, `/v1/models` discovery (+ llama.cpp `/props` runtime `n_ctx`, 2026-06-28); httptest-hermetic; replaces `Placeholder`. P1.2: the `Responder` seam is now streaming-only (`Stream`) — the loop consumes it; `Respond` stays a concrete `Client` method. 2026-07-27 ([ADR 0024](../adr/0024-the-heartbeat-observes-upstream-and-rebind-applies-at-the-boundary.md)): `SetModel` moves the client's configured wire model under a mutex (it outranks the request's model id, so a rebind must tell the client); the local server-process manager (`ServerManager`) shipped at P1.1 is **deleted** — never constructed in production, its liveness half superseded by the Heartbeat row below. **Upstream api key (2026-07-27):** the client's long-standing `WithAPIKey` construction option is now WIRED at every construction site — the session client (`agent.New`/`Resume`, sub-agents ride the parent's), the heartbeat Monitor, the host report's `probe.Discover`, and `probe model`'s two clients — from the `api-key:` config key / `APOGEE_API_KEY` (env > file, no flag: a secret must not reach shell history or `ps`). Passed unconditionally: an empty key sends no `Authorization` header, so a keyless local server is byte-identical to before; the key is redacted from server-echoed errors (`sanitize`) and `apogee probe` reports its PRESENCE only** | PID-file orphan adoption (deferred with local-server start/stop, TODO.md "[P1] Server / model switching") |
| Heartbeat / upstream monitor | ∅→**P (2026-07-27)** | **[ADR 0024](../adr/0024-the-heartbeat-observes-upstream-and-rebind-applies-at-the-boundary.md):** `internal/heartbeat` observes the Upstream every `Interval` (10 s, a named const — no config key) over `provider.Client.Discover` carrying the config `model:` as its **hint**, and reports a `Beat{Reachable, Failure, ActiveModel, ContextWindow, AvailableModels}` — never an error, because an unreachable server is a *finding* (the `probe.Discover` posture). Distinct noun from **Probe** ([ADR 0021](../adr/0021-probe-is-two-halves-the-host-report-is-free-the-model-battery-is-an-explicit-act.md)): a probe diagnoses once on demand for a human, a beat monitors continuously for the program. Two nil-able `tui.Options` seams split observe from apply — `Heartbeat` (the TUI imports `internal/heartbeat`, never `internal/provider`, ADR 0010) and `Rebind`, whose closure in `cmd/apogee/wire.go` re-resolves the per-model system prompt (ADR 0023), validated set (ADR 0016), mechanisms and window pin and drives `Agent.Rebind`. Tick chain re-arms from the **landed** beat (overlap impossible) with the spinner's generation counter; startup is fully async (first beat from `Init`, `Config.Model` may start empty, `errNoModelBound` guards `Submit`, late seed = the rebind path from empty); `Agent.Rebind` is **idle-only** — ADR 0011's engine-call class extended, deferred to the exchange-terminal fold when busy, so the loop's un-mutexed `cfg` reads need no lock — and validate-then-commit; offline debounce is busy-failure-ignored / cold-start-immediate / 2 consecutive idle failures, each crossing noted once, and blocks the three Exchange-opening paths with the typed input preserved | the `/model` + `/server` picker UI over `hb.models`, the endpoint switch, local llama.cpp start/stop (all TODO.md "[P1] Server / model switching"); gauge `TotalTokens` vs Budget prompt-side estimate and `/props` per-slot `n_ctx` (`total_slots` unread) are documented non-goals |
| processing/ (parsers) | ∅→**P (P3.5)** | RISKIEST; TS oracle + ported test vectors *is* the gate (0024b); **P1.3: one format end-to-end — native/JSON tool-call parse (`ParseNativeToolCalls`→`domain.ToolCall`, args validated, empty→`{}`, malformed→`ErrMalformedToolCall` never panic) + inline thinking-channel strip (`StripThinking`/`IsThinking`; delimited `<think>`, gpt-oss harmony); ported thinking-stripper vectors are the gate; package depends only on `domain`. P1.2: the loop adapts `provider.ToolCall`→`NativeToolCall` and parses at the seam (a malformed call degrades to a parse-error path, not a Turn failure). P3.5: parity finished — the two text formats behind the `ToolCallParser` interface (`MarkdownFencedParser` fenced-block + marker-fallback; `CustomRegexParser` named-group regex with JS→Go group rewrite + `{raw}` non-JSON fallback), `NewToolCallParser` processor-factory selecting native/markdown-fenced/custom-regex (native = text no-op; unknown format errors), and `StripHarmony`/`IsHarmonyThinking` for the full gpt-oss channel set (analysis/commentary/final; `<\|end\|>`/`<\|call\|>`/`<\|return\|>` terminators). All ported apogee-code TS vectors pass; malformed input degrades to the no-call path, never a panic. **Parser-seam wiring: `ParserFor(domain.ModelProfile)` translates the `Config.Profile` (Model profile) onto the frozen `ToolCallingConfig`/`ThinkingConfig` and returns the text `ToolCallParser` + a unified `ContentStripper` (none/delimited/harmony); the loop selects both once in `newAgent` and applies them at the parse seam — strip inline thinking/harmony, then recover a fenced/custom-regex call from the stripped content when the native path found none (native wins; text call gets a deterministic `text_call_<turn>` ID). A native/zero profile selects no-op parsers, so the content path is byte-identical. Prompt-seam (emit side): `InstructionsFor(ModelProfile, []ToolDef)` renders the text tool menu + format-specific tool-call instructions + a live example for a non-native profile — the request-seam mirror of `ParserFor`, driven by the same profile knobs/`withDefaults()` so what the model is told and what the loop parses cannot drift; a native/zero profile or an empty menu → `""`.** | live-stream channel suppression (the in-flight `TokenEvent` hold) is the following item; fenced marker overrides / probe auto-profiling are deferred (TODO.md) |
| Tools (21) | S→**P (P1.4)** | open extension point; stateless-across-Turns; external-effect boundary; the built-in suite `DefaultToolsWithHost` advertises is **21** — 19 unconditional plus `ask_user` and `present_document`, each registered only when its host delegate is supplied; **P1.4: minimal local set — `read_file`/`write_file`/`list_dir`/pure-Go `grep` (`io/fs` walk + `regexp`, no external programs) in `internal/tools/`, each scoped to a sandbox root at construction with traversal-rejecting path-safety (symlink-aware); real `domain.ToolRegistry`; `NewDefaultRegistry(root)` seam; optional `ReadOnlyTool` interface (the Plan-mode/Approval signal). P1.2: dispatch/approval/executor wired — `Config.WorkspaceDir` resolves the default registry; Plan filters the menu to read-only; allow-for-session cached; tool panics → `ErrorEvent` + error result. **P3.4: the per-call gate is now the D5 blast-radius disposition (mode × tool-class × confine-to-workspace × caps); `write_file` carries the unexported `workspaceScopedWriter` marker; subprocess tools (P3.8) get a `domain.Confinement` context handle to confine the cmd they build; external-effect tools route through `ExternalEffects.Do`. P3.7: the file-editing family — `single_find_and_replace`/`multi_find_and_replace` (exact-once, multi atomic), `edit_existing_file` (full-replace or `*** Begin Patch` hunks), `view_diff` (pure-Go Myers LCS, no external), `open_file` (read + substring-locate); ported from the apogee-code oracle, stateless (ADR 0008), each path-safe; the three writers carry the `workspaceScopedWriter` marker (diff/open-file are `ReadOnly`). P3.8: the execution tools — `terminal` (command line via `platform.Shell`, pre-flighted against the shell the line is actually bound for — `shlex`-validated on POSIX sh, no pre-parse for cmd.exe, whose grammar the POSIX splitter misreads (2026-07-22: `echo don't panic` and `dir "C:\Program Files\"` are valid cmd lines shlex rejects), the branch derived from `Shell.CommandLine` being empty; optional path-scoped `workdir`) and `python_exec` (detected `python3`/`python`, script on stdin, graceful when absent); both are `domain.SubprocessTool`s sharing `runSubprocess` which owns the §2.4 teardown (`Setpgid`+negative-PID-kill on cancel + `WaitDelay`, build-tagged unix/windows), the output cap, the timeout, and the `ConfinementFromContext` handoff; a runtime `ErrConfinementUnavailable` demotes the call to forced Approval in `dispatch.go` (re-run unconfined only on allow) — the "confine-if-you-can, gate-if-you-can't" RUNTIME net. P3.9: the git tools — `git_branch` (create/switch/list/delete; protected-branch delete block; safe `-d`), `git_commit` (path-safe staging + commit; amend refused on a published `origin/…` commit), `git_diff_range` (three-dot `base...head`; conservative ref-class validation; `ReadOnly`); ported from the apogee-code oracle, `domain.SubprocessTool`s on the shared `runSubprocess`, run with an allowlisted env (`safeEnvKeys`), detect the system `git` on PATH and degrade gracefully when absent (§3a, no hard dep). P3.10: the `diagnostics` tool — a read-only inspector that checks Go in-process (`go/parser` with `AllErrors` for syntax, dependency-free and toolchain-free) plus an optional `go vet` on the file's package (via the shared `runSubprocess`, allowlisted env, graceful "go vet skipped" note when no `go` on PATH), and returns a clear "no diagnostics available" for languages with no provider; declares `ReadOnly()` yet carries the `domain.SubprocessTool` marker (vet shells out) — the unfakeable marker outranks the self-declaration (contract §4, amended 2026-07-26 — url-safety H-1), so like `git_diff_range` it takes the **subproc** row: confined in Auto (gated when fs-confinement is unavailable), gated on the middle rungs, and a call **refuses** in Plan — and since 2026-08-02 both are also *withheld* from Plan's menu, whose filter keys on that same class rather than on the declaration (§4 footnote ², resolved); no new dependency (`go/parser`/`go/token` are stdlib). P3.11: the network + host tools — `web_fetch` (GET), `http_request` (method/headers/body, method arg-guard), `web_search` (a config'd `WebSearchEndpoint`; empty ⇒ the built-in **DuckDuckGo default**, `off` ⇒ graceful "disabled", never a crash) are in-process `net/http` `ExternalEffectTool`s of kind **network** (NOT `SubprocessTool`s, carry NO `workspaceScopedWriter`): the D5 disposition **auto-runs** them in Auto url-filtered and routes them through `ExternalEffects.Do` for the bench; each filters every URL through a `security.URLGuard` whose **default-on, resolved-IP SSRF floor** blocks loopback/private/IMDS/link-local **pre-flight AND at dial time** (`SafeDialControl` re-checks the connected IP — DNS-rebinding closed; no redirect auto-follow). `ask_user` routes a free-text question to the host's new `Asker` delegate — `ReadOnly()` (runs in Plan, mode-independent, never gated, NOT an `ExternalEffectTool`), registered **only** when an `Asker` is supplied (`NewDefaultRegistryWithHost` threads the `URLGuard`/endpoint/`Asker` from `Config`; the TUI implements `Asker` as an input-prompt rendezvous, the bench as a scripted responder); no new dependency (stdlib `net/http`). P3.15: external **MCP** server tools surface into the registry as `ExternalEffectTool{mcp}` (the `internal/mcp` client — see the MCP-client row); a discovered server tool is named `<server>__<tool>` and gates Approval in Auto via the existing `classMCP` disposition**. **`present_document` (2026-07-21, [ADR 0019](../adr/0019-documents-are-presented-not-opened.md)) — the deliverable-showing affordance and the 21st built-in: the model names an existing file (`path` + optional `title`) and the HOST picks the mechanism through the presentation ladder (transcript baseline → OS opener → doc-server URL → `present.command`), so the model never reasons about platforms and never supplies a command. It routes through the host's new `Presenter` delegate — the sibling of `ask_user`'s `Asker`: registered **only** when a `Presenter` is supplied (`NewDefaultRegistryWithHost` threads it from `Config`; the TUI walks the ladder, a headless host omits the tool), `ReadOnly()` so the disposition runs it freely in **every** mode including Plan, mode-independent (never the Approval gate), and **NOT** an `ExternalEffectTool` (the user's own display is not a non-forkable remote). The result text names the rung the host actually reached; a failed higher rung degrades to the baseline wording rather than reporting a failure the transcript contradicts.** | a user-tunable url-safety host allow/deny config key (deferred, TODO.md); ripgrep-optional |
| Context (Budget/Compaction/capping) | ∅→**P (P4 item 9 + overflow recovery)** | four-way split; Compaction default generative; capping = surviving half of `compress`; **P4 item 9:** `Allocate` splits the discovered window (response reserve / system / file / history) and `TokenEstimator` calibrates the chars→token ratio against each Turn's reported usage, so `LoopView.Budget()` is honest; the estimate-driven Compaction trigger folds at an Exchange boundary once history outgrows its allocation (S2: boundary-only, plus a saturation latch when a fold that RAN cannot help); `tool_result_cap` is the pre-request capping Mechanism (default-off). **Context-overflow recovery (2026-07-21, [ADR 0018](../adr/0018-context-overflow-recovers-structurally-the-emergency-fold-and-one-retry.md)):** a window overflow is now its own Turn outcome (`turnOverflowed`, its `ErrorEvent` withheld at the seam) and the STRUCTURAL `emergencyFold` — the one fold allowed to run MID-EXCHANGE — collapses history to protected prefix + summary + a user-role bridge (re-anchoring the cached `exchangeStart`) so the SAME Turn re-sends once; a predictive `requestExceedsWindow` guard folds *before* a request the estimate says cannot fit (threshold = the full working room, damped ×`uncalibratedRoomMargin` while the Budget is uncalibrated) and spends the same one-fold-per-Turn budget; `auto-compact: false` opts out of both, success is quiet (no new Event variant), and the give-up path is byte-identical to the pre-recovery behaviour. A **structural floor** clamps a single tool result estimated over the WHOLE History allocation as it enters the conversation (`appendToolResult`), sharing the head/tail-plus-marker rendering (`context.TruncateToolResult`) with the Mechanism. | token-aware trimming of `@file` refs + skill blocks at input time (§8 #8) |
...
| MCP client | ∅→**P (P3.15)** | official go-sdk **v1.6.1** (direct dep, pure-Go ⇒ cross-builds green); stdio/SSE/streamable-http; gates Approval in Auto. **P3.15 (`internal/mcp`): `Connect(ctx,[]ServerConfig,URLGuard)→*Client` dials every configured server, lists its tools, surfaces each as an `ExternalEffectTool{mcp}` named `<server>__<tool>` (so the existing D5 `classMCP` disposition gates it in Auto — NO dispatch change); `Tools()` registers them atop the default registry, `Close()` tears down (all-or-nothing Connect rollback, no orphan). Zero servers ⇒ dormant. SSE/streamable-http endpoints pass url-safety's scheme/host allow-deny and dial PINNED to the endpoint's own resolved addresses (the endpoint is exempt from the resolved-IP SSRF floor — it is config-file-only, never model-supplied — while any OTHER address on that connection still meets the floor; no redirect auto-follow) — **ADR 0012 Amendment (2026-07-26)**; stdio is a trusted local launch. Untrusted server description/schema/result shown to the model, never executed. Resume reconnects FRESH (ADR 0008). `cmd/apogee` wires `config.yaml`'s `mcp-servers:` (file-only, default-empty) → `mcp.Connect`; client shape → [`mcp-client.md`](mcp-client.md)** | server-grain "allow for session" cache (post-v1); user-tunable url-safety host allow/deny (TODO.md) |
...
| Platform (shell/path) | ∅→**P (P5.6/P5.7)** | one interface, POSIX v1, Windows Phase 5; **P5 item 6: the surface is `Shell{Command, CommandLine, Quote, ScopeEnv}` + `Path{Contains}`, and both rule sets live in ONE untagged table (`internal/platform/host.go`) compiled on every target — only `Current()`'s choice is build-tagged, so Windows semantics are table-tested from a Linux run. `CommandLine` returns the verbatim process command line for `SysProcAttr.CmdLine` ("" on POSIX): Windows has no argv at the syscall boundary and `os/exec`'s `EscapeArg` join produces quoting `cmd.exe` does not understand. `Quote` answers to BOTH parsers of that line — `CommandLineToArgvW` in the child (every backslash run that touches a quote is doubled) and `cmd.exe` in front of it (quote-toggling, metacharacters live outside a quoted region) — so a quote-free value is plainly double-quoted and a value carrying a quote of its own is caret-escaped, which is what keeps a quote plus an `&` from reaching `cmd` as a live command separator; the Phase-5 review pass replaced the old `""` escape, whose expected strings had been written down from the implementation rather than measured, and pinned the fix with a native `cmd /c` round-trip. `Contains` is the case-folded containment ADR 0020 needs — it normalises `\\?\`/`\\?\UNC\`, refuses drive-relative (`C:work`) and device (`\\.\…`) paths as non-locations, and treats an 8.3-shaped component as "resolve, else refuse" through an injected `longPath` seam (`GetLongPathNameW`, walking up to the longest existing prefix). **No `LookPath` wrapper** — `os/exec` already implements per-OS lookup including `%PATHEXT%`, so one would be dead surface; the executable suffix (`ExecExt`) was carried for a while as the one lookup-shaped fact `os/exec` does not expose, and was dropped again in the Phase-5 review pass for want of a production caller — it returns with the first one. Adopted callers: the `terminal` tool carries `spec.cmdline`, and `safeGitEnv` runs through `ScopeEnv` (POSIX output byte-identical — its floor is empty by design). **P5 item 7: process-*tree* teardown on Windows** — an unnamed Job Object created between `Start` and `Wait` with `KILL_ON_JOB_CLOSE`, terminated by `cmd.Cancel`; the decision half is the untagged `planTreeKill`, which the POSIX path now shares** | (none — both Phase-5 widening markers, and the teardown one, are retired) |
| TUI | ∅→**P (P2.x / P3.14)** | Bubble Tea; thin renderer over Events; supplies Approval delegate; **P3.14: sub-agent (`Depth > 0`) events now *render* (no longer just *tolerate*) — each nested block is framed by a `│ ` rail gutter per level and opened by a `⤷ sub-agent` label, derived purely from each event's `Depth` in the renderer (no Model state; ADR 0011 holds); the flat `Depth==0` transcript is byte-for-byte unchanged**. **Interjection staging (2026-07-27, [ADR 0025](../adr/0025-interjections-commit-at-the-between-steps-boundary.md)):** the TUI owns the whole queue and the engine only ever sees a delivery — `internal/tui/interject.go` holds the per-Exchange `interjectBox` (a mutex-guarded FIFO, by POINTER on the value-copied Model, pushed by the `Update` goroutine and drained by the worker between Steps — the one shared state in the feature) plus the staging half: `⏎` while running stages a row instead of submitting (the single-worker invariant untouched), Backspace on an empty box withdraws the newest row back into the editor, `interjectedMsg` moves exactly the delivered rows into the transcript as a distinct `entryInterjected` kind (⧖, user-styled, deliberately NOT a sticky header), and the terminal folds rule on the remainder — auto-flush as ONE joined unmarked message on `exchangeDoneMsg`/`compactDoneMsg`, hold under a single note on `cancelledMsg`/`errMsg`. Key routing, paste and mouse-caret placement all share ONE predicate, `inputEditable()` (idle / ask / running), so keyboard and mouse cannot disagree; the cost is the single-key transcript scroll while running (PgUp/PgDn + wheel remain). Also landed with it and needing no ADR: the **real terminal cursor** (virtual cursor retired, `View` attaches `textarea.Cursor()` translated by `inputContentRect`, hidden where the box is inert, never blinking, shape from the file-only `cursor-shape:` key parsed by `tui.ParseCursorShape`) and the **keep-if-unchanged** selection rule (`transcriptSel.spanUnchanged` in `refreshViewport` — a transcript selection lives while every line it spans is unchanged, so a drag survives the stream and copy equals sight) | panes; live token gauge (Phase 4) |
| CLI / `headless` / `probe` | ∅→**P (P5.1/P5.3/P5.4)** | Cobra; headless optional (NOT bench contract); probe doubles as fingerprint; **P5 item 1: the root accepts subcommands and bare `apogee` stays byte-identical — `maybeDispatchConfinedExec` is still the first thing `main` does, `Args: cobra.NoArgs` is retained on the root RunE, and the seam shipped EMPTY (so `--help` gained its Commands section only when `probe` landed). P5 items 3–4 ([ADR 0021](../adr/0021-probe-is-two-halves-the-host-report-is-free-the-model-battery-is-an-explicit-act.md)): `probe` is a parent whose own RunE prints the **host report** — free, offline, read-only (OS/arch, backend + capability matrix + `AutoEligible()`, effective `confine-to-workspace` after the Host acknowledgement, roots, endpoint reachability, whether an upstream api key was resolved — presence, never the value — `/v1/models` + `/props`, and a Windows label-journal residue line) — with `probe host` as the scriptable twin and `probe model` as the **explicit act**: a three-part capability battery (native tool call, JSON output, multi-step chain) that prints findings + an ordinal capability tier + paste-ready profile knobs, and WRITES a versioned probe record. The selection/notice logic is EXTRACTED into `internal/probe` (`BackendName`/`DegradedNotice`/`CapabilityLine`), which `/confine status` renders too, so CLI and TUI cannot word one verdict two ways. Per ADR 0021's 2026-07-22 Amendment the behavioral tier promotes the **advertised label** to `ConfidenceMedium` and files the feature vector beside it as a separate behavioral **signature** — evidence, never a match key; the resolver ladder is `library.ResolveFingerprintFrom` (High → Medium probe record → Low)** | `headless` (still deferred — the skeleton merely makes it possible); adaptive prompt complexity (recorded in TODO.md, bench-gated per ADR 0009) |
...
5. ✅ **Provider/Upstream client** — **DONE (P1.1):** `internal/provider.Client` (non-streaming `Respond` + streaming `Stream`, bounded retries/timeouts), `/v1/models` discovery, `ServerManager`; httptest-hermetic, replaces `Placeholder`. TS oracle ported (`openai-compatible-provider` / `model-discovery` / `server-process-manager`). *(`ServerManager` was deleted 2026-07-27 — see the §5 Provider row and [ADR 0024](../adr/0024-the-heartbeat-observes-upstream-and-rebind-applies-at-the-boundary.md).)*
6. ✅ **processing/ — one tool-call format** — **DONE (P1.3):** native/JSON tool-call parse (`ParseNativeToolCalls`→`domain.ToolCall`; empty args→`{}`; malformed→`ErrMalformedToolCall`, never panic) + inline thinking-channel strip (`StripThinking`/`IsThinking`; delimited `<think>`, gpt-oss harmony `<|channel|>…<|end|>`). **Finding:** the bench (apogee-sim) and the deliverable run on native structured `tool_calls` (grammar-forced JSON when a server lacks support), so "the most common native/JSON tool-call shape" is literal; the provider already extracts the wire shape and keeps args verbatim, so processing parses args + strips thinking. Ported apogee-code thinking-stripper vectors are the parity gate; the package depends only on `domain` (loop adapts `provider.ToolCall`→`NativeToolCall` at the seam — ADR 0010). markdown-fenced/custom-regex + full harmony channels are Phase 3; loop wiring is P1.2.
...
8. ✅ **Context reducers: Budget allocation, Compaction trigger/strategy, tool-result capping, token counting** — **DONE (P4 item 9; overflow recovery 2026-07-21):** `Allocate` + the usage-calibrated `TokenEstimator` make the Budget honest; Compaction has two automatic triggers — estimate-driven at an Exchange boundary, and the overflow-driven **emergency fold** that may run mid-Exchange and lets the Turn re-send once, with a predictive pre-request guard sharing its one fold per Turn ([ADR 0018](../adr/0018-context-overflow-recovers-structurally-the-emergency-fold-and-one-retry.md)); `tool_result_cap` caps oversized results pre-request, above a structural loop-side floor on a single result over the whole History allocation. **Still open:** token-aware trimming of `@file` refs and skill blocks at *input* time (the context-builder concern; `maxRefFileBytes` is only a sanity bound).
